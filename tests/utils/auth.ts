...
export const TEST_PERMISSIONS = {
  USER: ['profile:read', 'profile:update', 'learning:read', 'learning:update'],
  PREMIUM: [
    'profile:read',
    'profile:update',
    'learning:read',
...
export const mockSignIn = vi.fn().mockImplementation(async (provider?: string, options?: any) => {
  return {
    error: undefined,
    status: 200,
    ok: true,
...
export const mockSignOut = vi.fn().mockImplementation(async (options?: any) => {
  return {
    url: options?.callbackUrl || '/',
  }
})
