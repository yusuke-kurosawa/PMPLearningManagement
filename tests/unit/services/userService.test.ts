...
describe('UserService', () => {
  ...
  describe('updateUser', () => {
    ...
    test('should prevent duplicate email during update', async () => {
      ...
      await expect(UserService.updateUser(user2.id, updateData)).rejects.toThrow(
        expect.objectContaining({
          code: 'CONFLICT',
          message: 'このメールアドレスは既に使用されています',
        })
    ...
    test('should validate Twitter handle format', async () => {
      ...
      await expect(UserService.updateUser(user.id, validUpdate)).resolves.not.toThrow()
  ...
  describe('changeUserRole', () => {
    test('should change user role successfully', async () => {
      ...
      const result = await UserService.changeUserRole(user.id, UserRole.PREMIUM, 'admin-123')
    ...
    test('should prevent changing last admin role', async () => {
      ...
      await expect(UserService.changeUserRole(adminUser.id, UserRole.USER)).rejects.toThrow(
        expect.objectContaining({
          code: 'FORBIDDEN',
