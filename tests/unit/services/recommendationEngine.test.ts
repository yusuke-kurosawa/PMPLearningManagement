...
describe('Learning Recommendation Engine', () => {
  ...
  describe('Personalized Content Recommendation', () => {
    ...
    it('should consider user progress and mastery levels', async () => {
      ...
      const highMasteryContent = recommendations.filter(
        (r) => r.processId === 'process-1' && r.difficulty === 'ADVANCED'
      )
  ...
  describe('Collaborative Filtering', () => {
            ...
            data: [
              {
                userId: user.id,
                processId: 'process-1',
                masteryLevel: faker.number.int({ min: 70, max: 90 }),
                studyTime: faker.number.int({ min: 2400, max: 4800 }),
                lastStudied: new Date(),
              },
              {
                userId: user.id,
                processId: 'process-2',
                masteryLevel: faker.number.int({ min: 65, max: 85 }),
                studyTime: faker.number.int({ min: 1800, max: 3600 }),
                lastStudied: new Date(),
              },
              {
                userId: user.id,
                processId: 'process-4',
                masteryLevel: faker.number.int({ min: 80, max: 95 }),
                studyTime: faker.number.int({ min: 3000, max: 4200 }),
                lastStudied: new Date(),
              },
            ],
          })
    ...
    it('should recommend content based on similar users preferences', async () => {
      ...
      const collaborativeRecommendations =
        await learningRecommendationService.getCollaborativeRecommendations(testUser.id)
    ...
    it('should weight recommendations by user similarity', async () => {
      const collaborativeRecs = await learningRecommendationService.getCollaborativeRecommendations(
        testUser.id
      )

      // 類似度の高いユーザーからの推奨ほど高スコア
      const sortedRecs = collaborativeRecs.sort(
        (a, b) => b.recommendationScore - a.recommendationScore
      )

      expect(sortedRecs[0].recommendationScore).toBeGreaterThanOrEqual(
        sortedRecs[sortedRecs.length - 1].recommendationScore
      )
    })
  })
  ...
  describe('Content-Based Filtering', () => {
    it('should recommend similar content to previously consumed', async () => {
      ...
      const contentBasedRecs = await learningRecommendationService.getContentBasedRecommendations(
        testUser.id
      )
    ...
    it('should balance between exploration and exploitation', async () => {
      ...
      const balancedRecs = await learningRecommendationService.getPersonalizedRecommendations(
        testUser.id,
        {
  ...
  describe('Adaptive Learning Path Generation', () => {
    ...
    it('should adapt path based on user progress', async () => {
      ...
      // パスを更新
      const adaptedPath = await adaptiveLearningService.adaptLearningPath(
        testUser.id,
        initialPath.id
      )
      ...
      const completedStepExists = adaptedPath.steps.some(
        (step) => step.processId === initialPath.steps[0].processId && step.status === 'COMPLETED'
      )
  ...
  describe('Real-time Recommendation Updates', () => {
    it('should update recommendations after user interaction', async () => {
      // 初期推奨
      const initialRecs = await learningRecommendationService.getPersonalizedRecommendations(
        testUser.id
      )
      ...
      // 推奨を再取得
      const updatedRecs = await learningRecommendationService.getPersonalizedRecommendations(
        testUser.id
      )

      // 低評価されたタイプのコンテンツは減る
      const lowRatedTypeContent = updatedRecs.filter(
        (r) =>
          r.type === (await prisma.learningContent.findUnique({ where: { id: 'content-1' } }))?.type
      )
      const initialLowRatedTypeContent = initialRecs.filter(
        (r) =>
          r.type === (await prisma.learningContent.findUnique({ where: { id: 'content-1' } }))?.type
      )
    ...
    it('should incorporate immediate feedback', async () => {
      ...
      const feedbackAdjustedRecs =
        await learningRecommendationService.getPersonalizedRecommendations(testUser.id)

      // ポジティブフィードバックを受けたコンテンツと類似のものが上位に
      const helpfulContentType = (
        await prisma.learningContent.findUnique({ where: { id: 'content-5' } })
      )?.type
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle recommendation system failures gracefully', async () => {
      ...
      // フォールバック推奨が提供される
      const fallbackRecs = await learningRecommendationService.getPersonalizedRecommendations(
        testUser.id
      )
