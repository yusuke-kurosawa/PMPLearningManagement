...
describe('Progress Analytics - Learning Progress Calculation', () => {
  ...
  describe('Basic Progress Calculation', () => {
    ...
    it('should calculate weighted progress based on process complexity', async () => {
      ...
      const simpleWeighted = await progressAnalyticsService.calculateWeightedProgress(
        testUser.id,
        false
      )
      const complexWeighted = await progressAnalyticsService.calculateWeightedProgress(
        testUser.id,
        true
      )
            ...
            data: [
              {
                userId: peer.id,
                processId: 'process-1',
                masteryLevel: faker.number.int({ min: 60, max: 90 }),
                studyTime: faker.number.int({ min: 1800, max: 4800 }),
                lastStudied: new Date(),
              },
              {
                userId: peer.id,
  ...
  describe('Error Handling', () => {
    ...
    it('should handle database connection errors', async () => {
      ...
      prisma.learningProgress.findMany = vi
        .fn()
        .mockRejectedValue(new Error('Database connection failed'))

      await expect(progressAnalyticsService.calculateOverallProgress(testUser.id)).rejects.toThrow(
    ...
    it('should handle invalid input parameters', async () => {
      ...
      for (const invalidInput of invalidInputs) {
        await expect(
          progressAnalyticsService.calculateOverallProgress(invalidInput as any)
