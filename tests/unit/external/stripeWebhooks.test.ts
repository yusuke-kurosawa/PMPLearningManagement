...
describe('Stripe Webhooks - Event Processing', () => {
  ...
  describe('Subscription Events', () => {
    ...
    it('should handle customer.subscription.created webhook', async () => {
      ...
      // ウェルカムメールが送信される
      expect(emailService.sendWelcomeEmail).toHaveBeenCalledWith(testCustomer.email, expect.any(Object));
    });
    ...
    it('should handle customer.subscription.updated webhook', async () => {
      ...
      // 支払い問題の通知メールが送信される
      expect(emailService.sendPaymentIssueNotification).toHaveBeenCalledWith(testCustomer.email, expect.any(Object));
    });
    ...
    it('should handle customer.subscription.deleted webhook', async () => {
      ...
      // キャンセル確認メールが送信される
      expect(emailService.sendCancellationConfirmation).toHaveBeenCalledWith(testCustomer.email, expect.any(Object));
  ...
  describe('Error Handling and Resilience', () => {
    ...
    it('should handle partial failures in complex events', async () => {
      ...
      // メール送信のみ失敗
      vi.mocked(emailService.sendPlanChangeNotification).mockRejectedValue(new Error('Email service unavailable'));
