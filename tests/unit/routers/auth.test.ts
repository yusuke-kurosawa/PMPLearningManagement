...
const authRouter = {
  ...
  createCaller: (ctx: any) => ({
    ...
    register: async (input: { name: string; email: string; password: string }) => {
      const existingUser = testDb.findMany('user', {}).find((u: any) => u.email === input.email)
    ...
    changePassword: async (input: { currentPassword: string; newPassword: string }) => {
      ...
      const isCurrentValid = await mockBcrypt.compare(input.currentPassword, user.hashedPassword)
...
describe('Auth Router', () => {
  ...
  describe('refreshToken', () => {
    ...
    test('should reject invalid refresh token', async () => {
      ...
      await expect(caller.refreshToken({ refreshToken: 'invalid_token' })).rejects.toThrow(
        expect.objectContaining({
          code: 'UNAUTHORIZED',
          message: 'Invalid refresh token',
        })
    ...
    test('should reject wrong token type', async () => {
      ...
      await expect(caller.refreshToken({ refreshToken: 'wrong_type_token' })).rejects.toThrow(
        expect.objectContaining({
          code: 'UNAUTHORIZED',
        })
      )
    ...
    test('should reject token for deleted user', async () => {
      ...
      await expect(caller.refreshToken({ refreshToken })).rejects.toThrow(
        expect.objectContaining({
          code: 'UNAUTHORIZED',
        })
      )
  ...
  describe('verifyEmail', () => {
    ...
    test('should reject invalid token', async () => {
      ...
      await expect(caller.verifyEmail({ token: 'invalid_token' })).rejects.toThrow(
        expect.objectContaining({
          code: 'BAD_REQUEST',
          message: 'Invalid or expired verification token',
        })
    ...
    test('should reject wrong token type', async () => {
      ...
      await expect(caller.verifyEmail({ token: 'wrong_type_token' })).rejects.toThrow(
        expect.objectContaining({
          code: 'BAD_REQUEST',
        })
      )
