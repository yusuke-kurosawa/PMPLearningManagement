...
const userRouter = {
  createCaller: (ctx: any) => ({
    ...
    update: async (input: {
      ...
      if (input.email && input.email !== user.email) {
        const existingUser = testDb.findMany('user', {}).find(
          (u: any) => u.email === input.email && u.id !== input.id
        );
      ...
      const updatedUser = testDb.update('user', { id: input.id }, {
    ...
    delete: async (input: { id: string }) => {
      ...
      if (user.role === 'ADMIN') {
        const adminCount = testDb.findMany('user', {}).filter(
          (u: any) => u.role === 'ADMIN'
        ).length;
      ...
      // Soft delete
      testDb.update('user', { id: input.id }, {
...
describe('User Router', () => {
  ...
  describe('getStats', () => {
    test('should return user statistics for admin', async () => {
      ...
      const users = [
        ...
        createUser({ role: 'PREMIUM', lastLoginAt: new Date(Date.now() - 40 * 24 * 60 * 60 * 1000) }),
  ...
  describe('Edge Cases', () => {
    ...
    test('should validate input parameters', async () => {
      ...
      await expect(
        caller.changeRole({ id: user.id, role: 'INVALID_ROLE' })
      ).resolves.toBeDefined(); // In this mock, we don't validate
    });
  });
});