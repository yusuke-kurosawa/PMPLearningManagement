...
describe('RBAC - Role Management', () => {
  ...
  describe('Permission Checking', () => {
    ...
    it('should handle contextual permissions', async () => {
      ...
      // 自分のリソースにはアクセス可能
      const canEditOwnNotes = await rbacService.hasPermission(
        user, 
        'notes.edit', 
        context
      );
      ...
      const canEditOthersNotes = await rbacService.hasPermission(
        user,
        'notes.edit',
        otherContext
      );
    ...
    it('should handle resource quotas', async () => {
      ...
      // 制限回数分は実行可能
      for (let i = 0; i < 3; i++) {
        const canTakeExam = await rbacService.hasPermission(
          user, 
          'exam.take_limited',
          { quotaCheck: true }
        );
      ...
      // 制限を超えた場合
      const canTakeMoreExams = await rbacService.hasPermission(
        user,
        'exam.take_limited', 
        { quotaCheck: true }
      );
    ...
    beforeEach(async () => {
      ...
      await prisma.course.createMany({
        data: [
          ...
          { id: 'course-2', title: 'Private Course', visibility: 'PRIVATE', ownerId: 'instructor-1' },
          { id: 'course-3', title: 'Premium Course', visibility: 'PREMIUM', ownerId: 'instructor-2' }
        ]
      });
    });
