...
describe('Security - JWT Token Management', () => {
  ...
  describe('JWT Token Verification', () => {
    ...
    it('should handle concurrent token verification', async () => {
      ...
      const results = await Promise.all(tokens.map((token) => authService.verifyAccessToken(token)))
...
describe('Security - Password Management', () => {
  describe('Password Hashing', () => {
    ...
    it('should handle empty or null passwords', async () => {
      ...
      await expect(securityService.hashPassword(null as any)).rejects.toThrow(
        'Password must be a string'
      )
      await expect(securityService.hashPassword(undefined as any)).rejects.toThrow(
        'Password must be a string'
      )
    })

    it('should enforce password complexity requirements', async () => {
      const weakPasswords = ['123456', 'password', 'abc123', '111111', 'qwerty']
...
describe('Security - Vulnerability Testing', () => {
  ...
  describe('XSS Protection', () => {
    ...
    it('should preserve safe HTML while removing dangerous content', () => {
      const mixedContent =
        '<p>Safe content</p><script>alert("xss")</script><strong>More safe content</strong>'
