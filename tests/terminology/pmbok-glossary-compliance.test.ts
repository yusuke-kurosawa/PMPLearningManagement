...
class PMBOKGlossaryComplianceChecker {
  ...
  private calculateConsistencyScore(): number {
    const inconsistencyViolations = this.violations.filter(
      (v) => v.rule === 'inconsistent-terminology' || v.rule === 'alternative-term-used'
    ).length

    const totalTermUsages = this.violations.length
    if (totalTermUsages === 0) {
      return 100
    }

    return Math.max(
      0,
      Math.round(((totalTermUsages - inconsistencyViolations) / totalTermUsages) * 100)
    )
  }

  private calculateAccuracyScore(): number {
    const accuracyViolations = this.violations.filter((v) => v.severity === 'error').length
  ...
  private generateSuggestions(): string[] {
    ...
    if (inconsistentTerms.length > 0) {
      ...
      suggestions.push(
        `Standardize usage of these inconsistent terms: ${uniqueTerms.slice(0, 5).join(', ')}`
      )
    }
  ...
  describe('Critical Terminology Errors', () => {
    it('should have no critical errors in core components', () => {
      ...
      const criticalErrors =
        complianceReport.violations?.filter(
          (v) => v.severity === 'error' && criticalFiles.some((pattern) => v.file.includes(pattern))
        ) || []
    ...
    it('should properly capitalize PMBOK terminology', () => {
      const capitalizationErrors =
        complianceReport.violations?.filter((v) => v.rule === 'improper-capitalization') || []
    ...
    it('should use standard PMBOK acronyms', () => {
      const acronymErrors =
        complianceReport.violations?.filter((v) => v.rule === 'non-standard-acronym') || []
  ...
  describe('Process Group Terminology', () => {
    ...
    processGroups.forEach((pg) => {
      it(`should correctly reference ${pg} process group`, () => {
        const pgViolations =
          complianceReport.violations?.filter(
            (v) => v.context.toLowerCase().includes(pg.toLowerCase()) && v.severity === 'error'
          ) || []
  ...
  describe('ITTO Framework Compliance', () => {
    ...
    it('should maintain consistent ITTO terminology format', () => {
      ...
      const formatViolations =
        complianceReport.violations?.filter((v) => v.rule === 'itto-format-inconsistency') || []
  ...
  describe('Component-Specific Tests', () => {
    describe('Learning Components', () => {
      it('should use correct learning terminology in educational components', async () => {
        ...
        const learningViolations =
          complianceReport.violations?.filter(
            (v) => learningFiles.includes(v.file) && v.severity === 'error'
          ) || []
    ...
    describe('Visualization Components', () => {
      it('should use consistent PMBOK process terminology in visualizations', async () => {
        ...
        const vizViolations =
          complianceReport.violations?.filter(
            (v) => vizFiles.includes(v.file) && v.rule === 'process-terminology'
          ) || []
  ...
  describe('Documentation Compliance', () => {
    it('should maintain PMBOK standards in README documentation', () => {
      const readmeViolations =
        complianceReport.violations?.filter(
          (v) => v.file.includes('README.md') && v.severity === 'error'
        ) || []
    ...
    it('should maintain consistency in CLAUDE.md project documentation', () => {
      const claudeViolations =
        complianceReport.violations?.filter(
          (v) => v.file.includes('CLAUDE.md') && v.severity === 'error'
        ) || []
    ...
    it('should use proper terminology in API documentation', () => {
      const apiDocViolations =
        complianceReport.violations?.filter(
          (v) => v.file.includes('docs/api/') && v.severity === 'error'
        ) || []
...
export function generateDetailedTestReport(report: ComplianceReport): string {
  const sections = [
    ...
    '',
    '### By Type',
    ...Object.entries(report.violationsByType).map(([type, count]) => `- ${type}: ${count}`),
    '',
    '### By File',
...
export { PMBOKGlossaryComplianceChecker, createMockViolation, expectValidViolation, TEST_CONFIG }
