...
async function mergeCoverageReports(): Promise<void> {
  try {
    ...
    // 統合HTMLレポート生成
    execSync(`npx c8 report --reporter=html --reports-dir=${coverageDir}/merged --out-dir=${coverageDir}/html`, { stdio: 'inherit' });
    execSync(`npx c8 report --reporter=json-summary --reports-dir=${coverageDir}/merged --out-dir=${coverageDir}`, { stdio: 'inherit' });
...
function evaluateQualityGate(testResults: any, coverageStats: any, performanceMetrics: any): boolean {
  return (
