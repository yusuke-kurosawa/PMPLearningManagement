...
class DataIntegrityManager {
  ...
  private async validateConstraints(
    ...
    const applicableConstraints = Array.from(this.constraints.values()).filter(
      (c) => c.table === table && c.isEnabled
    )
  ...
  private async validateUnique(
    ...
    for (const [existingId, existingData] of tableData) {
      ...
      const hasConflict = constraint.columns.every(
        (column) =>
          existingData[column] === data[column] &&
          data[column] !== null &&
      ...
      if (hasConflict) {
        throw new Error(
          `UNIQUE constraint violated: ${constraint.table}.${constraint.columns.join(',')}`
        )
      }
    }
  }
  ...
  private async validateForeignKey(
    ...
    for (const [_, referencedData] of referencedTableData) {
      const matches = constraint.referencedColumns!.every(
        (column, index) => referencedData[column] === referencedValues[index]
      )
    ...
    if (!referencedRecordExists) {
      throw new Error(
        `FOREIGN KEY constraint violated: ${constraint.table} -> ${constraint.referencedTable}`
      )
    }
  }
  ...
  private async checkForeignKeyReferences(constraint: ConstraintRule, data: any): Promise<void> {
    ...
    const dependentConstraints = Array.from(this.constraints.values()).filter(
      (c) => c.type === 'FOREIGN_KEY' && c.referencedTable === constraint.table
    )
    ...
    for (const depConstraint of dependentConstraints) {
      ...
      for (const [_, dependentData] of dependentTableData) {
        const isReferencing = depConstraint.columns.every(
          (column, index) => dependentData[column] === referencingValues[index]
        )

        if (isReferencing) {
          throw new Error(`Cannot delete: record is referenced by ${depConstraint.table}`)
        }
      }
    }
  }
  ...
  async performIntegrityCheck(): Promise<IntegrityCheckResult> {
    ...
    for (const [table, tableData] of this.data) {
      ...
      for (const [recordId, record] of tableData) {
        ...
        const tableConstraints = Array.from(this.constraints.values()).filter(
          (c) => c.table === table && c.isEnabled
        )
  ...
  private async checkForOrphanedRecords(corruptions: DataCorruption[]): Promise<void> {
    const foreignKeyConstraints = Array.from(this.constraints.values()).filter(
    ...
    for (const constraint of foreignKeyConstraints) {
      ...
      for (const [recordId, record] of tableData) {
        ...
        for (const [_, referencedRecord] of referencedTableData) {
          const matches = constraint.referencedColumns!.every(
            (column, index) => referencedRecord[column] === referencedValues[index]
          )
  ...
  async simulateCorruption(
    ...
    switch (type) {
      ...
      case 'ORPHANED_RECORD':
        ...
        const foreignKeyConstraints = Array.from(this.constraints.values()).filter(
          (c) => c.type === 'FOREIGN_KEY' && c.table === table
        )
        ...
        for (const constraint of foreignKeyConstraints) {
          if (constraint.referencedTable) {
            ...
            for (const [refId, refRecord] of referencedTableData) {
              const matches = constraint.referencedColumns!.every(
                (column, index) => refRecord[column] === referencedValues[index]
              )
...
describe('Data Consistency Guarantees - Advanced Testing', () => {
  ...
  describe('Constraint Validation', () => {
    ...
    it('should prevent deletion of referenced records', async () => {
      ...
      await expect(manager.deleteRecord('users', 'user1')).rejects.toThrow(
  ...
  describe('Corruption Detection', () => {
    ...
    it('should detect duplicate key violations', async () => {
      ...
      const uniqueViolation = integrityCheck.violations.find(
        (v) => v.violationType === 'UNIQUE' || v.violationType === 'PRIMARY_KEY'
      )
  ...
  describe('Data Repair', () => {
    ...
    it('should repair orphaned records', async () => {
      ...
      // Verify corruption is resolved
      const integrityCheck = await manager.performIntegrityCheck()
      const orphanCorruptions = integrityCheck.corruptions.filter(
        (c) => c.type === 'ORPHANED_RECORD'
      )
      ...
      fc.asyncProperty(
        ...
        async (operations) => {
          ...
          for (const violation of integrityCheck.violations) {
            expect(['CHECK', 'FOREIGN_KEY', 'UNIQUE', 'PRIMARY_KEY']).toContain(
  ...
  describe('Advanced Consistency', () => {
    it('should handle cascading constraint checks', async () => {
      ...
      await expect(manager.deleteRecord('user_progress', 'progress1')).rejects.toThrow(
        'Cannot delete: record is referenced'
      )
  ...
  describe('Recovery Edge Cases', () => {
    it('should handle multiple simultaneous corruptions', async () => {
      ...
      expect(integrityCheck.isValid).toBe(false)
      expect(
        integrityCheck.corruptions.length + integrityCheck.violations.length
