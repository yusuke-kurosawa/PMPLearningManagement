...
class ConcurrencyController {
  ...
  private removeLockRequest(request: LockRequest): void {
    const lock = this.locks.get(request.resourceId)
    if (lock) {
      lock.waitQueue = lock.waitQueue.filter(
        (req) =>
    ...
    if (nextRequest) {
      ...
      if (transaction && transaction.status === 'ACTIVE') {
        ...
        if (nextRequest.lockType === 'SHARED') {
          const compatibleRequests = lock.waitQueue.filter(
            (req) =>
              req.lockType === 'SHARED' &&
...
describe('Concurrency Control - Advanced Testing', () => {
  ...
  describe('Deadlock Management', () => {
    it('should detect simple deadlock scenarios', async () => {
      ...
      const deadlockDetected = results.some(
        (result) =>
          result.status === 'rejected' && (result.reason as Error).message.includes('Deadlock')
      )
    ...
    it('should function correctly with deadlock detection disabled', async () => {
      ...
      const timeoutErrors = results.filter(
        (result) =>
          result.status === 'rejected' && (result.reason as Error).message.includes('timeout')
      ).length
  ...
  describe('Lock Management', () => {
    ...
    it('should queue exclusive lock requests properly', async () => {
      ...
      const grantedLocks = results.filter(
  ...
  describe('Performance and Stress', () => {
    it('should handle high-concurrency scenarios', async () => {
      ...
      const connections = Array.from({ length: numTransactions }, () =>
          ...
          (async function () {
            try {
              ...
              await controller.acquireLock(
                tx,
                resourceId,
                Math.random() > 0.5 ? 'SHARED' : 'EXCLUSIVE'
              )
  ...
  describe('Error Handling', () => {
    it('should handle invalid transaction operations', async () => {
      ...
      await expect(controller.acquireLock('invalid_tx', 'resource1', 'SHARED')).rejects.toThrow(
        'Invalid or inactive transaction'
      )

      // Try to commit non-existent transaction
      await expect(controller.commitTransaction('invalid_tx')).rejects.toThrow(
    ...
    it('should prevent operations on committed/aborted transactions', async () => {
      ...
      await expect(controller.acquireLock(tx, 'resource1', 'SHARED')).rejects.toThrow(
        'Invalid or inactive transaction'
      )
    })
    ...
    it('should handle resource cleanup after timeout', async () => {
      ...
      // This should timeout
      await expect(controller.acquireLock(tx2, 'timeout_resource', 'EXCLUSIVE')).rejects.toThrow(
        'Lock timeout'
      )
