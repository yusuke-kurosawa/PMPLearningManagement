...
class BusinessRuleEngine {
  ...
  private extractVariablesFromFormula(formula: string): string[] {
    ...
    return [...new Set(matches)].filter(
      (match) => !['Math', 'parseInt', 'parseFloat', 'Number'].includes(match)
    )
  }
            ...
            actions: [
              {
                ...
                value: null,
                parameters: { validator: 'range', min: 0, max: 100 },
              },
            ],
            priority: 100,
            isActive: true,
  ...
  describe('Complex Conditional Logic', () => {
    it('should handle nested AND/OR conditions correctly', () => {
      engine.addRule({
        ...
        conditions: [
          ...
          {
            field: 'progress.certificationProgress',
            operator: 'lt',
            value: 0.9,
            logicalOperator: 'OR',
          },
        ...
        actions: [{ type: 'SET_FIELD', target: 'user.eligible', value: true }],
        priority: 50,
        isActive: true,
      })
    ...
    it('should handle regex conditions properly', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'user.id', operator: 'regex', value: '^[a-zA-Z0-9]+$' }],
        actions: [
    ...
    it('should handle contains operations with case sensitivity', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'session.actions', operator: 'contains', value: 'QUIZ' }],
        actions: [{ type: 'SET_FIELD', target: 'session.hasQuiz', value: true }],
        priority: 60,
        isActive: true,
      })
  ...
  describe('Action Execution', () => {
    it('should perform complex calculations', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'session.completed', operator: 'eq', value: true }],
        actions: [
          {
            type: 'CALCULATE',
    ...
    it('should generate template-based notifications', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'user.studyStreak', operator: 'gte', value: 7 }],
        actions: [
          {
            type: 'NOTIFY',
            target: 'streak_achievement',
            value:
              'Congratulations {{user.id}}! You have maintained a {{user.studyStreak}}-day study streak!',
          },
        ],
        priority: 40,
        isActive: true,
    ...
    it('should handle blocking actions', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'user.subscriptionTier', operator: 'eq', value: 'free' }],
        actions: [
  ...
  describe('Error Handling', () => {
    ...
    it('should handle malformed conditions gracefully', () => {
      engine.addRule({
        ...
        conditions: [{ field: 'nonexistent.field', operator: 'eq', value: 'test' }],
        actions: [{ type: 'LOG', target: 'test', value: 'Test log' }],
        priority: 50,
        isActive: true,
      })
  ...
  describe('Boundary Values', () => {
    it('should handle extreme numeric values', () => {
      ...
      engine.addRule({
        ...
        conditions: [{ field: 'user.averageScore', operator: 'gt', value: 0 }],
        actions: [
          {
            type: 'CALCULATE',
            target: 'test.result',
    ...
    it('should handle empty and null values', () => {
      ...
      engine.addRule({
        ...
        conditions: [{ field: 'user.completedProcesses', operator: 'eq', value: [] }],
        actions: [{ type: 'SET_FIELD', target: 'user.isNewbie', value: true }],
        priority: 50,
        isActive: true,
      })
    ...
    it('should handle very large rule sets', () => {
      ...
      for (let i = 0; i < 100; i++) {
        engine.addRule({
          ...
          conditions: [{ field: 'user.studyStreak', operator: 'gte', value: i % 10 }],
          actions: [{ type: 'LOG', target: `test_${i}`, value: `Rule ${i} executed` }],
          priority: i,
