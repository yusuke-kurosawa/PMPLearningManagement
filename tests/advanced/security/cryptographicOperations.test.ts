...
class CryptographicManager {
  ...
  async encryptRSA(publicKeyId: string, plaintext: ArrayBuffer): Promise<EncryptedData> {
    ...
    if (plaintext.byteLength > maxPlaintextLength) {
      throw new Error(`Plaintext too large for RSA key size: ${plaintext.byteLength} > ${maxPlaintextLength}`);
    }
  ...
  async hash(data: ArrayBuffer, algorithm: 'SHA-256' | 'SHA-384' | 'SHA-512' = 'SHA-256'): Promise<HashResult> {
    const startTime = performance.now();
  ...
  getActiveKeys(): EncryptionKey[] {
    return Array.from(this.keys.values())
      .filter(key => key.isActive && (!key.expiresAt || key.expiresAt > new Date()));
  }
...
describe('Cryptographic Operations - Advanced Security Testing', () => {
  ...
  describe('AES-GCM Encryption', () => {
    ...
    it('should fail with invalid key', async () => {
      ...
      await expect(
        cryptoManager.encryptAESGCM('invalid_key', plaintext)
      ).rejects.toThrow('Invalid or inactive AES-GCM key');
    });
    ...
    it('should fail with tampered ciphertext', async () => {
      ...
      await expect(
        cryptoManager.decryptAESGCM(encrypted)
  ...
  describe('RSA Encryption', () => {
    ...
    it('should reject plaintext too large for key size', async () => {
      ...
      await expect(
        cryptoManager.encryptRSA(keyPair.publicKey.id, largePlaintext)
      ).rejects.toThrow('Plaintext too large for RSA key size');
    });
    ...
    it('should fail decryption with wrong private key', async () => {
      ...
      await expect(
        cryptoManager.decryptRSA(keyPair2.privateKey.id, encrypted)
  ...
  describe('Digital Signatures', () => {
    ...
    it('should fail verification with tampered data', async () => {
      ...
      const isValid = await cryptoManager.verifySignature(keyPair.publicKey.id, signature, tamperedData);
  ...
  describe('Timing Safety', () => {
    ...
    it('should resist timing attacks on comparison', async () => {
      const correctHash = await cryptoManager.hash(stringToArrayBuffer('correct_password'), 'SHA-256');
  ...
  describe('Key Management', () => {
    ...
    it('should handle expired keys', async () => {
      ...
      await expect(
        cryptoManager.encryptAESGCM(key.id, plaintext)
  ...
  describe('Performance Metrics', () => {
    ...
    it('should perform timing analysis for potential attacks', async () => {
      ...
      const decryptOperation = () => cryptoManager.decryptRSA(keyPair.privateKey.id, validCiphertext);
    ...
    await fc.assert(
      fc.asyncProperty(
        ...
        async (data, algorithm) => {
          ...
          const isValid = await cryptoManager.verifySignature(keyPair.publicKey.id, signature, dataBuffer);
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle corrupted key material gracefully', async () => {
      ...
      await expect(
        cryptoManager.encryptAESGCM('non_existent_key', plaintext)
      ).rejects.toThrow('Invalid or inactive AES-GCM key');
    });
  });
});