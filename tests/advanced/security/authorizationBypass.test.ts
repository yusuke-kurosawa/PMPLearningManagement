...
class AuthorizationManager {
      ...
      permissions: [
        {
          ...
          resource: 'users',
          actions: ['read', 'update'],
          conditions: [{ field: 'level', operator: 'less_than', value: 50 }],
        },
        {
          id: 'mod_content',
          resource: 'content',
    ...
    const userRole: Role = {
      ...
      permissions: [
        ...
        },
        {
          ...
          conditions: [{ field: 'ownerId', operator: 'equals', value: '${user.id}' }],
        },
        {
          id: 'user_public_read',
          resource: 'content',
          actions: ['read'],
          conditions: [{ field: 'visibility', operator: 'equals', value: 'public' }],
        },
      ],
    }
  ...
  private async detectBypassAttempts(context: AccessContext): Promise<BypassAttempt | undefined> {
    const suspiciousPatterns: Array<{
      ...
      {
        pattern: (ctx) => ctx.resource === 'roles' || (ctx.resourceData && ctx.resourceData.roles),
      ...
      // Privilege escalation
      {
        pattern: (ctx) => {
          const userMaxLevel = Math.max(
            ...ctx.user.roles.map((roleId) => this.roles.get(roleId)?.level || 0)
          )
  ...
  ): Promise<'ALLOW' | 'DENY' | 'AUDIT' | 'CONTINUE'> {
  ...
  getBypassAttempts(limit: number = 50): AccessAttempt[] {
    return this.accessAttempts.filter((attempt) => attempt.bypassAttempt).slice(-limit)
  }
  ...
  getUserRoles(userId: string): Role[] {
    ...
    return user.roles.map((roleId) => this.roles.get(roleId)).filter(Boolean) as Role[]
  }
