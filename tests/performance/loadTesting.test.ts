...
describe('Performance Testing - API Response Times', () => {
  ...
  describe('API Endpoint Performance', () => {
    ...
    it('should handle recommendation engine under load', async () => {
      ...
      const averageTime =
        recommendationTimes.reduce((sum, time) => sum + time, 0) / recommendationTimes.length
      const p95Time = recommendationTimes.sort((a, b) => a - b)[
        Math.floor(recommendationTimes.length * 0.95)
      ]
    ...
    it('should handle exam data queries with complex joins efficiently', async () => {
      ...
      const averageQueryTime =
        examQueryTimes.reduce((sum, time) => sum + time, 0) / examQueryTimes.length
  ...
  describe('Concurrent User Load Testing', () => {
    ...
    it('should handle burst traffic patterns', async () => {
      ...
      const performanceDegradation =
        burstResults[burstResults.length - 1].avgTime / burstResults[0].avgTime
