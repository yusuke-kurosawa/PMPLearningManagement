...
export class SelfHealingAutomation {
  ...
  getHealingStatistics() {
    ...
    const totalFailures = Array.from(this.failureHistory.values()).reduce((sum, count) => sum + count, 0);
    ...
    return {
      ...
      averageRecoveryTime: Array.from(this.successfulRecoveries.values())
        .reduce((sum, r) => sum + r.timeSpent, 0) / totalRecoveries
    };
  }
