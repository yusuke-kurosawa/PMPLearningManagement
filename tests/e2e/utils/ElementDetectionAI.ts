...
export class ElementDetectionAI {
   ...
   */
  private async contextualDetection(
    context?: string,
    description?: string
  ): Promise<ElementDetectionResult> {
   ...
   */
  private async similarityBasedDetection(
    previousSelectors: string[]
  ): Promise<ElementDetectionResult> {
  ...
  private extractSemanticFeatures(description: string): any {
    ...
    features.keywords = words.filter(
      (word) =>
  ...
  private isPositionMatch(box: any, position: any): boolean {
    const tolerance = 10
    return Math.abs(box.x - position.x) < tolerance && Math.abs(box.y - position.y) < tolerance
  }
  ...
  private generateContextualSelectors(
    pageContext: any,
    context: string,
    description: string
  ): any[] {
