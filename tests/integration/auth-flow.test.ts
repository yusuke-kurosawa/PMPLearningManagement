...
class AuthFlowService {
  async register(userData: { name: string; email: string; password: string }) {
    ...
    const existingUser = testDb.findMany('user', {}).find((u: any) => u.email === userData.email)
  ...
  async login(credentials: { email: string; password: string }) {
    const user = testDb.findMany('user', {}).find((u: any) => u.email === credentials.email)
    ...
    const isValidPassword = await mockBcrypt.compare(credentials.password, user.hashedPassword)
    ...
    const refreshToken = mockJwt.sign({ sub: user.id, type: 'refresh' }, 'test-secret', {
      expiresIn: '7d',
    })
  ...
  async refreshToken(refreshToken: string) {
    ...
    const newRefreshToken = mockJwt.sign({ sub: user.id, type: 'refresh' }, 'test-secret', {
      expiresIn: '7d',
    })
  ...
  async changePassword(userId: string, currentPassword: string, newPassword: string) {
    ...
    const isValidPassword = await mockBcrypt.compare(currentPassword, user.hashedPassword)
  ...
  async resetPassword(email: string) {
    const user = testDb.findMany('user', {}).find((u: any) => u.email === email)
    ...
    const resetToken = mockJwt.sign({ sub: user.id, type: 'password_reset' }, 'test-secret', {
      expiresIn: '1h',
    })
  ...
  async confirmPasswordReset(token: string, newPassword: string) {
    ...
    const resetRecord = testDb
      .findMany('passwordReset', { userId: decoded.sub })
...
describe('Authentication Flow Integration', () => {
  ...
  describe('Complete Registration Flow', () => {
    test('should complete full registration process', async () => {
      ...
      // Step 2: Verify email
      const verificationResult = await authService.verifyEmail(registrationResult.verificationToken)
      ...
      const user = testDb.findMany('user', {}).find((u: any) => u.email === 'john@example.com')
  ...
  describe('Token Refresh Flow', () => {
    test('should refresh tokens successfully', async () => {
      ...
      const user = testDb.findMany('user', {}).find((u: any) => u.email === 'token@example.com')
      const session = testDb.findMany('session', { userId: user.id })[0]
    ...
    test('should reject wrong token type', async () => {
      ...
      await expect(authService.refreshToken('wrong_type_token')).rejects.toThrow(
        'Invalid token type'
      )
    })
  })
  ...
  describe('Password Management Flow', () => {
    ...
    test('should reject incorrect current password', async () => {
      ...
      await expect(
        authService.changePassword(loginResult.user.id, 'WrongPass123!', 'NewPass123!')
  ...
  describe('Password Reset Flow', () => {
    ...
    test('should reject expired reset tokens', async () => {
      ...
      await expect(authService.confirmPasswordReset(expiredToken, 'NewPass123!')).rejects.toThrow(
  ...
  describe('Error Handling and Edge Cases', () => {
    test('should handle malformed tokens gracefully', async () => {
      const malformedTokens = ['not-a-jwt-token', 'malformed.jwt.token', '', null, undefined]
      ...
      for (const token of malformedTokens) {
        ...
        await expect(authService.refreshToken(token as string)).rejects.toThrow()
      }
    })
