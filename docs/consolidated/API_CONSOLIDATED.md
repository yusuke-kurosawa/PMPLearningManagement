...
<!-- Consolidated on: 2025-08-09T15:12:24.889Z -->
<!-- Source files: BACKEND_ARCHITECTURE.md, API_SPECIFICATION.md -->
...
_Source: `docs/api/BACKEND_ARCHITECTURE.md`_
...
export const progressRouter = createTRPCRouter({
  ...
  updateProgress: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      return await ProgressService.updateProgress(ctx.session.user.id, input)
    }),
        ...
        processProgress: {
          where: {
            ...(filters?.knowledgeArea && {
              processId: {
                in: PMBOK_PROCESSES.filter((p) => p.knowledgeArea === filters.knowledgeArea).map(
                  (p) => p.id
                ),
              },
            }),
...
export class LearningProgress {
  ...
  getOverallProgress(): number {
    const completed = Array.from(this.processes.values()).filter((p) => p.isCompleted).length
    return (completed / this.processes.size) * 100
  }

  getKnowledgeAreaProgress(area: string): number {
    const areaProcesses = Array.from(this.processes.values()).filter(
      (p) => PMBOK_PROCESSES.find((pm) => pm.id === p.processId)?.knowledgeArea === area
    )
...
export const paymentRouter = createTRPCRouter({
  ...
  createSubscription: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await PaymentService.createSubscription(ctx.session.user.id, input.planType)
    }),
      ...
      messages: [
        {
          role: 'system',
          content:
            'あなたはPMP試験対策の専門講師です。ユーザーの学習進捗に基づいて個別の学習提案を行います。',
        },
        {
          role: 'user',
          content: prompt,
...
export class EmailService {
  ...
  static async sendWelcomeEmail(user: any) {
    return await this.sendEmail(user.email, 'PMP Learning へようこそ！', 'welcome', {
      name: user.name || 'さん',
    })
  }
  ...
  static async sendProgressReport(user: any, report: any) {
    return await this.sendEmail(user.email, '週間学習レポートをお届けします', 'progress_report', {
      name: user.name || 'さん',
      report,
    })
  }
  ...
  static async calculateUserStats(userId: string) {
    ...
    const knowledgeAreaStats = await prisma.$queryRaw`
      ...
      JOIN (VALUES ${Prisma.join(
        PMBOK_PROCESSES.map((p) => Prisma.sql`(${p.id}, ${p.knowledgeArea})`)
...
describe('ProgressService', () => {
  ...
  describe('getProgress', () => {
    ...
    it('存在しないユーザーの場合はエラーを投げる', async () => {
      ...
      // Act & Assert
      await expect(ProgressService.getProgress(mockUserId)).rejects.toThrow(
...
export async function GET() {
  ...
  const isHealthy = Object.values(results.checks).every((check) => check.status === 'ok')
...
_Source: `docs/API_SPECIFICATION.md`_
