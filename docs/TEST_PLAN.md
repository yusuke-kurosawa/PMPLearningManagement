...
describe('Security Tests', () => {
  ...
  describe('Authorization Security', () => {
    it('権限のないエンドポイントへのアクセスを拒否', async () => {
      ...
      await expect(
        userClient.admin.getSystemHealth.query()
    ...
    it('他ユーザーのデータへの不正アクセスを防ぐ', async () => {
      ...
      await expect(
        user1Client.user.getById.query({ id: user2Id })
      ).rejects.toThrow('Forbidden')
    })
  })
})
...
describe('Infrastructure Tests', () => {
  it('RDSインスタンスが適切に設定されている', async () => {
    ...
    const rdsConfig = plan.configuration.root_module.resources
      .find(r => r.type === 'aws_db_instance')
