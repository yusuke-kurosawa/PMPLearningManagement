...
import {
  ...
  Cell, Area, AreaChart, PieChart, Pie, Funnel, FunnelChart
} from 'recharts'
import {
...
import {
  ...
  BarChart3, Activity, Target, Users, Clock, Hash, Zap,
  FileText, Award, AlertCircle, RefreshCw, Download, Filter,
  ChevronUp, ChevronDown, Eye, Edit, Trash2, Copy
} from 'lucide-react'
...
const QuestionAnalyticsDashboard: React.FC = () => {
  ...
  const summaryMetrics = useMemo(() => {
    ...
    const averageDiscrimination = questions.reduce((sum, q) => sum + q.discriminationIndex, 0) / totalQuestions
    
    const highQualityCount = questions.filter(q => q.qualityScore >= 0.8).length
    const lowPerformingCount = questions.filter(q => q.correctRate < 0.3 || q.correctRate > 0.9).length
    ...
    for (const domain of domains) {
      for (const difficulty of difficulties) {
        ...
        if (relevantQuestions.length > 0) {
          const avgDiscrimination = relevantQuestions.reduce(
            (sum, q) => sum + q.discriminationIndex, 0
                    ...
                    {['easy', 'medium', 'hard'].map(difficulty => {
                      ...
                      return (
                        <div
                          ...
                          className="h-20 rounded flex flex-col items-center justify-center text-white font-medium"
                          style={{
            ...
            <Card>
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart 
                    data={[
                      { range: '0-20%', count: questions.filter(q => q.qualityScore < 0.2).length },
                      { range: '20-40%', count: questions.filter(q => q.qualityScore >= 0.2 && q.qualityScore < 0.4).length },
                      { range: '40-60%', count: questions.filter(q => q.qualityScore >= 0.4 && q.qualityScore < 0.6).length },
                      { range: '60-80%', count: questions.filter(q => q.qualityScore >= 0.6 && q.qualityScore < 0.8).length },
                      { range: '80-100%', count: questions.filter(q => q.qualityScore >= 0.8).length }
                    ]}
                  >
                    <CartesianGrid strokeDasharray="3 3" />
                    <XAxis dataKey="range" />
              ...
              <ResponsiveContainer width="100%" height={300}>
                <FunnelChart>
                  ...
                  <Funnel
                    dataKey="value"
                    data={[
                      { name: 'Remember', value: questions.filter(q => q.bloomsLevel === 'remember').length, fill: '#8884d8' },
                      { name: 'Understand', value: questions.filter(q => q.bloomsLevel === 'understand').length, fill: '#83a6ed' },
                      { name: 'Apply', value: questions.filter(q => q.bloomsLevel === 'apply').length, fill: '#8dd1e1' },
                      { name: 'Analyze', value: questions.filter(q => q.bloomsLevel === 'analyze').length, fill: '#82ca9d' },
                      { name: 'Evaluate', value: questions.filter(q => q.bloomsLevel === 'evaluate').length, fill: '#a4de6c' },
                      { name: 'Create', value: questions.filter(q => q.bloomsLevel === 'create').length, fill: '#ffc658' }
                    ]}
                    isAnimationActive
                  />
                </FunnelChart>
        ...
        <TabsContent value="irt" className="space-y-4">
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={irtCurveData}>
                      ...
                      <XAxis 
                        dataKey="ability" 
                        domain={[-3, 3]}
                        label={{ value: 'Student Ability (θ)', position: 'insideBottom', offset: -5 }}
                      />
                      <YAxis 
                        domain={[0, 1]}
                  ...
                  <AreaChart 
                    data={Array.from({ length: 61 }, (_, i) => {
                      ...
                      questions.forEach(q => {
                        ...
                        const info = Math.pow(q.irtA, 2) * Math.pow(p - q.irtC, 2) / 
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Discrimination (a), Difficulty (b), and Guessing (c) parameters</CardDescription>
            </CardHeader>
            <CardContent>
                  ...
                  <TableBody>
                    {questions.slice(0, 20).map(question => (
                      ...
                      >
                        ...
                        <TableCell>
                          {(Math.max(...Array.from({ length: 61 }, (_, i) => {
                            const theta = -3 + i * 0.1
                            const p = question.irtC + (1 - question.irtC) / 
                                     (1 + Math.exp(-question.irtA * (theta - question.irtB)))
                            return Math.pow(question.irtA, 2) * Math.pow(p - question.irtC, 2) / 
                                  (Math.pow(1 - question.irtC, 2) * p * (1 - p))
                          }).filter(v => !isNaN(v)))).toFixed(2)}
                        </TableCell>
                        <TableCell>
                          <Button
                            size="sm"
                      ...
                      <TableRow key={question.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            <Progress 
                              value={question.qualityScore * 100} 
                              className="w-16"
                            />
