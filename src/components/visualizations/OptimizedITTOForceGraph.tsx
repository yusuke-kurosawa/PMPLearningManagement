...
const OptimizedITTOForceGraph: React.FC<OptimizedITTOForceGraphProps> = React.memo(
  ({
    ...
    const renderCanvas = useCallback(() => {
      ...
      if (!canvas || !context || !simulationRef.current) {
        return
      }
      ...
      context.translate(transform.x * devicePixelRatio, transform.y * devicePixelRatio)
      context.scale(transform.k * devicePixelRatio, transform.k * devicePixelRatio)
    ...
    const renderSVG = useCallback(() => {
      ...
      const linkSelection = g
        ...
        .data(processedData.links, (d: Link) => `${(d.source as Node).id}-${(d.target as Node).id}`)
    ...
    useEffect(() => {
      if (!isVisible || !processedData.nodes.length) {
        return
      }

      // Create simulation with optimized parameters
      const simulation = d3
        .forceSimulation<Node>(processedData.nodes)
        .force(
          'link',
          d3
            .forceLink<Node, Link>(processedData.links)
            .id((d: any) => d.id)
        ...
        .force(
          'center',
          d3.forceCenter(dimensions.width / 2, dimensions.height / 2).strength(centerStrength)
        )
      ...
      if (enableClustering) {
        simulation.force('cluster', d3.forceCluster().strength(0.2))
      }
      ...
      if (enableZoom) {
        const zoom = d3
          ...
          .on(
            'zoom',
            throttle((event: d3.D3ZoomEvent<any, any>) => {
