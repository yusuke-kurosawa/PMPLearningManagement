...
import { talentTriangleData, analyzeTalentTriangleBalance, TalentTriangleAssessment } from '../../data/pmbok/pmiTalentTriangleData';
...
const PMITalentTriangle: React.FC<PMITalentTriangleProps> = ({ className = '' }) => {
  ...
  const [userAssessment, setUserAssessment] = useState<Partial<TalentTriangleAssessment> | null>(null);
  ...
  // 三角形の視覚化コンポーネント
  const TriangleVisualization: React.FC<{ assessment?: TalentTriangleAssessment }> = ({ assessment }) => {
    const size = 300;
      ...
      <div className="flex justify-center">
        <svg width={size} height={size} className="drop-shadow-md">
          ...
          {talentTriangleData.map((category, index) => {
            ...
            if (index === 0) labelY -= labelOffset; // 上
            else if (index === 1) { // 左下
              labelX -= labelOffset;
              labelY += labelOffset;
            } else { // 右下
            ...
            <CardContent>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  {talentTriangleData.map((category) => {
                    const Icon = iconMap[category.icon as keyof typeof iconMap];
                    const categoryAssessment = userAssessment?.categories.find(c => c.categoryId === category.id);
                ...
                <Tabs defaultValue="overview" className="w-full">
                  ...
                  <TabsContent value="skills" className="space-y-4">
                    {selectedSkillData && (
                      <div className="space-y-6">
                        <div>
                          <h3 className="text-xl font-semibold mb-3">{selectedSkillData.name}</h3>
                          <p className="text-gray-600 mb-4">{selectedSkillData.importance}</p>
                            ...
                            <div>
                              ...
                              <ul className="space-y-2">
                                {selectedSkillData.examples.map((example, index) => (
                                  <li key={index} className="flex items-center text-sm">
                                    <Circle className="h-2 w-2 mr-2 fill-current text-blue-500" />
                                    {example}
                                  </li>
                                ))}
                              </ul>
                        ...
                        <div>
                          <h4 className="font-semibold mb-3">スキルレベル評価基準</h4>
                              ...
                              <Card key={index}>
                                <CardContent className="p-4">
                                  <div className="flex items-center space-x-3 mb-2">
                                    <Badge 
                                      variant={
                                        level.level === '初級' ? 'secondary' :
                                        level.level === '中級' ? 'default' : 'destructive'
                                      }
                                    >
                                      {level.level}
                                    </Badge>
                  ...
                  <TabsContent value="development" className="space-y-4">
                    {selectedSkillData && (
                      <div>
                        <h3 className="text-xl font-semibold mb-4">開発のヒント</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <Card>
                          <CardContent className="p-4 text-center">
                            <div className="text-lg font-semibold text-green-600">
                              {talentTriangleData.find(c => c.id === analysis.strongestArea)?.name}
                            </div>
