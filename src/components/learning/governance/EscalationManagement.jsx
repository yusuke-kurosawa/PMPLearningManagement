...
const EscalationManagement = () => {
  ...
  const updateThreshold = (type, level, value) => {
    setThresholds((prev) => ({
      ...prev,
      [type]: prev[type].map((threshold, index) => (index === level ? value : threshold)),
    }))
  }
  ...
  const exportEscalationMatrix = () => {
    const matrix = {
      ...
      configuration: {
        ...
        averageResponseTime:
          escalationLevels.reduce((acc, level) => acc + level.responseTime, 0) /
  ...
  const getSeverityColor = (value, type) => {
    if (type === 'budget' || type === 'schedule') {
      if (value < thresholds[type][0]) {
        return 'text-green-600'
      }
      if (value < thresholds[type][1]) {
        return 'text-yellow-600'
      }
      if (value < thresholds[type][2]) {
            ...
            <TabsContent value='levels' className='space-y-6'>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>階層的なエスカレーション構造と権限レベルを定義</CardDescription>
                </CardHeader>
                        ...
                        <div className='grid gap-3 md:grid-cols-2 lg:grid-cols-4'>
                          {level.escalationCriteria.map((criteria, index) => (
                            <div key={index} className='rounded-lg bg-gray-50 p-3'>
                              ...
                              <p
                                className={`text-lg font-bold ${getSeverityColor(criteria.threshold, criteria.type)}`}
                              >
                                {criteria.threshold}
                                {criteria.unit}
                              </p>
            ...
            <TabsContent value='matrix' className='space-y-6'>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>各レベルのしきい値とトリガー条件を調整</CardDescription>
                </CardHeader>
                            ...
                            <div key={index} className='space-y-2'>
                              ...
                              {type === 'budget' || type === 'schedule' ? (
                                <Input
                                  ...
                                  value={value}
                                  onChange={(e) =>
                                    updateThreshold(type, index, parseInt(e.target.value))
                                  }
                                  className='w-full'
                                />
                              ) : (
                                <Select
                                  ...
                                  onValueChange={(newValue) =>
                                    updateThreshold(type, index, newValue)
                                  }
                                >
                                  <SelectTrigger>
                                    <SelectValue />
            ...
            <TabsContent value='simulation' className='space-y-6'>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>様々なシナリオでエスカレーション過程を実演</CardDescription>
                </CardHeader>
                <CardContent className='space-y-6'>
                  <div className='grid gap-4 md:grid-cols-3'>
                    {scenarios.map((scenario) => (
                      <Card key={scenario.id} className='border-2 border-gray-200'>
                        <CardContent className='p-4'>
                          <h3 className='mb-2 font-medium text-gray-900'>{scenario.name}</h3>
                            ...
                            >
                              <div className='flex items-center gap-2'>
                                ...
                                <span className='text-sm'>
                                  ...
                                  {activeScenario.type === 'budget' ||
                                  activeScenario.type === 'schedule'
                                    ? '%'
                                    : ''}
                                </span>
                              </div>
            ...
            <TabsContent value='analytics' className='space-y-6'>
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>エスカレーション履歴の分析とトレンド</CardDescription>
                </CardHeader>
                <CardContent className='space-y-6'>
                  <div className='grid gap-6 lg:grid-cols-2'>
                    <div>
                      <h3 className='mb-4 font-medium text-gray-900'>月次エスカレーション履歴</h3>
                    ...
                    <div>
                      <h3 className='mb-4 font-medium text-gray-900'>エスカレーション分布</h3>
                      <ResponsiveContainer width='100%' height={300}>
                        <PieChart>
                          <Pie
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      ...
                      レベル1での解決率が高く、良好なエスカレーション管理が行われています。
