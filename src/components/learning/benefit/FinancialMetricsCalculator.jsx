...
const FinancialMetricsCalculator = () => {
  ...
  const calculateMetrics = useCallback((project) => {
    ...
    const irr = financialCalc.calculateIRR(
      project.initialInvestment,
      project.cashFlows
    )
    ...
    const costs = new Array(project.cashFlows.length).fill(project.initialInvestment / project.cashFlows.length)
              ...
              {projects.map(project => (
                <div
                  ...
                  className={`flex items-center justify-between p-2 rounded cursor-pointer transition-colors ${
                    activeProjectId === project.id ? 'bg-blue-50 border border-blue-200' : 'hover:bg-gray-50'
                  }`}
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateProject(activeProject.id, 'initialInvestment', parseFloat(e.target.value) || 0)}
                    className="mt-1"
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => updateProject(activeProject.id, 'discountRate', parseFloat(e.target.value) || 0)}
                    className="mt-1"
                  />
                </div>
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => changeCashFlowYears(activeProject.id, parseInt(value))}
                    >
                      ...
                      return (
                        <Card key={key} className="p-3">
                          <div className="flex items-start justify-between">
                            ...
                            {evaluation.label && (
                              <Badge
                                variant={evaluation.level === 'excellent' ? 'default' : 
                                        evaluation.level === 'good' ? 'secondary' :
                                        evaluation.level === 'fair' ? 'outline' : 'destructive'}
                              >
                                {evaluation.label}
                              </Badge>
                            )}
                        ...
                        <div className="h-64">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={comparisonChartData}>
                              ...
                              <XAxis dataKey="name" />
                              <YAxis />
                              <Tooltip formatter={(value) => financialCalc.formatFinancialValue(value, 'currency')} />
                              <Legend />
                      ...
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          <AreaChart data={cashFlowChartData}>
                            ...
                            <Tooltip formatter={(value) => financialCalc.formatFinancialValue(value, 'currency')} />
                            <Legend />
                            <Area type="monotone" dataKey="キャッシュフロー" stackId="1" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.6} />
                            <Line type="monotone" dataKey="累積キャッシュフロー" stroke="#ef4444" strokeWidth={2} />
                          </AreaChart>
                        </ResponsiveContainer>
                      </div>
                    </CardContent>
                      ...
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          <RadarChart data={radarChartData}>
                            ...
                            <Radar name="評価" dataKey="value" stroke="#3b82f6" fill="#3b82f6" fillOpacity={0.6} />
                          </RadarChart>
                        </ResponsiveContainer>
                      </div>
                    </CardContent>
                  ...
                  <div className="h-64">
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart data={sensitivityAnalysis}>
                        ...
                        <YAxis />
                        <Tooltip
                          formatter={(value) => financialCalc.formatFinancialValue(value, 'currency')}
                          labelFormatter={(label) => `変動率: ${label}%`}
                        />
                        <Legend />
                        <Line type="monotone" dataKey="npv" stroke="#3b82f6" name="NPV" strokeWidth={2} />
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                        ...
                        <Alert>
                          ...
                          <AlertDescription>
                            <div className="space-y-2">
                              ...
                              <p>
                                <strong>期待NPV:</strong>{' '}
                                {financialCalc.formatFinancialValue(simulationResults.mean, 'currency')}
                              </p>
                              <p>
                                <strong>中央値:</strong>{' '}
                                {financialCalc.formatFinancialValue(simulationResults.median, 'currency')}
                              </p>
                              <p>
                                <strong>95%信頼区間:</strong>{' '}
                                {financialCalc.formatFinancialValue(simulationResults.percentile5, 'currency')} ～{' '}
                                {financialCalc.formatFinancialValue(simulationResults.percentile95, 'currency')}
                              </p>
                            </div>
                          </AlertDescription>
                        </Alert>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <Card className="p-3">
                            ...
                            <div className="text-lg font-semibold mt-1">
                              {financialCalc.formatFinancialValue(simulationResults.standardDeviation, 'currency')}
                            </div>
                          </Card>
                          <Card className="p-3">
