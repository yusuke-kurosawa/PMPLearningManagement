...
const OKRManagement = () => {
  ...
  const statusConfig = {
    'on-track': { label: '順調', color: 'text-green-600', bgColor: 'bg-green-50', icon: CheckCircle2 },
    'at-risk': { label: 'リスク', color: 'text-yellow-600', bgColor: 'bg-yellow-50', icon: AlertCircle },
  ...
  const saveOkr = () => {
    if (!newObjective.trim() || newKeyResults.some(kr => !kr.trim())) return
          ...
          if (kr.id === keyResultId) {
            ...
            return {
              ...
              status: progressPercent >= 100 ? 'completed' : 
                     ...
                     progressPercent >= 40 ? 'at-risk' : 'off-track'
            }
          }
          return kr
        })
        
        const overallProgress = updatedKeyResults.reduce((acc, kr) => 
          acc + (kr.current / kr.target * 100), 0) / updatedKeyResults.length
  ...
  const calculatePeriodStats = () => {
    if (filteredOkrs.length === 0) return { avgProgress: 0, completed: 0, onTrack: 0, atRisk: 0 }
    ...
    const statusCounts = filteredOkrs.reduce((acc, okr) => {
      const mainStatus = okr.progress >= 100 ? 'completed' : 
                        okr.progress >= 70 ? 'onTrack' : 'atRisk'
      acc[mainStatus]++
      return acc
      ...
      <CardContent className="space-y-6">
          ...
          <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
            ...
            <DialogContent className="max-w-2xl">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700">主要成果 (Key Results)</label>
        ...
        {filteredOkrs.length === 0 ? (
          <Card className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">OKRが設定されていません</h3>
            <p className="text-gray-500 mb-4">新しいOKRを作成して価値実現を追跡しましょう</p>
                ...
                <CardContent>
                  <div className="space-y-3">
                    ...
                    {okr.keyResults.map((kr) => {
                      ...
                      return (
                        <div key={kr.id} className="bg-gray-50 p-3 rounded-lg space-y-2">
                          <div className="flex items-start justify-between">
                            <p className="text-sm text-gray-700 flex-1">{kr.description}</p>
                            <Badge 
                              variant="outline" 
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div className="text-xs text-gray-600 min-w-20 text-right">
                          ...
                          <div className="flex items-center justify-between text-xs text-gray-500">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Input
                                type="number"
                                value={kr.current}
                                onChange={(e) => updateKeyResultProgress(okr.id, kr.id, Number(e.target.value))}
