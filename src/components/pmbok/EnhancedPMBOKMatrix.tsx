...
const EnhancedPMBOKMatrix: React.FC = () => {
  ...
  const getMasteryColor = (level: UserProgress['masteryLevel']) => {
    switch (level) {
      ...
      case 'familiar': return 'bg-yellow-100 text-yellow-800 border-yellow-300';
  ...
  const getProgressStats = () => {
    ...
    const studied = Object.values(userProgress).filter(p => p.masteryLevel !== 'not_started').length;
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <Card className="w-96">
          <CardContent className="p-6 text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">Loading PMBOK Data</h3>
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-gray-600">
                  Interactive guide to PMBOK {selectedVersion === 'both' ? '6th & 7th' : `${selectedVersion}th`} Edition processes
                </p>
              </div>
            ...
            <Card className="mb-6">
              <CardContent className="p-4">
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  {/* Version Toggle */}
                  <Select value={selectedVersion.toString()} onValueChange={(value) => setSelectedVersion(value as any)}>
                  ...
                  <Select value={selectedKnowledgeArea} onValueChange={setSelectedKnowledgeArea}>
                    ...
                    <SelectContent>
                      ...
                      {KNOWLEDGE_AREAS.map(ka => (
                        <SelectItem key={ka} value={ka}>{ka}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div className="flex flex-wrap items-center gap-4 mt-4">
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      checked={showOnlyUnstudied}
                      onCheckedChange={setShowOnlyUnstudied}
                    />
          ...
          {viewMode === 'matrix' ? (
            ...
            <div className="overflow-x-auto">
              <div className="min-w-[1200px]">
                <div className="grid grid-cols-6 gap-2">
                  ...
                  {PROCESS_GROUPS.map(pg => (
                    <div key={pg} className="font-semibold text-sm text-gray-700 p-2 text-center bg-gray-100 rounded">
                      {pg}
                    </div>
                  ))}
                  ...
                  {KNOWLEDGE_AREAS.map(ka => (
                    <React.Fragment key={ka}>
                      <div className="font-semibold text-sm text-gray-700 p-2 bg-gray-100 rounded min-h-[100px] flex items-center">
                        <span className="transform -rotate-90 whitespace-nowrap">
                          {ka.replace('Project ', '')}
                        </span>
                      </div>
                      {PROCESS_GROUPS.map(pg => (
                        <div key={`${ka}-${pg}`} className="min-h-[100px] border border-gray-200 rounded p-2">
                          {processMatrix[ka][pg].map(process => {
                            ...
                            return (
                              <div
                                key={process.id}
                                className={`mb-2 last:mb-0 rounded border transition-all duration-200 ${
                                  progress?.masteryLevel ? getMasteryColor(progress.masteryLevel) : 'bg-white border-gray-200'
                                  ...
                                  <div className="flex items-start justify-between">
                                    <div className="flex-1 min-w-0">
                                      ...
                                      <div className="flex items-center gap-1 mt-1">
                                        ...
                                        <Badge 
                                          variant={process.difficulty === 'advanced' ? 'destructive' : 
                                                 process.difficulty === 'intermediate' ? 'default' : 'secondary'}
                                          className="text-xs"
                                        >
                                          {process.difficulty}
                                        </Badge>
                                  ...
                                  {isExpanded && (
                                    <div className="mt-2 pt-2 border-t border-gray-200 space-y-2">
                                      ...
                                      <div className="text-xs">
                                        ...
                                        {process.inputs.filter(i => i.isKey).map(i => i.name).join(', ')}
                                      </div>
                                      
                                      <div className="text-xs">
                                        <strong>Key Outputs:</strong> {' '}
                                        {process.outputs.filter(o => o.isKey).map(o => o.name).join(', ')}
                                      </div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                              ...
                              <ul className="text-sm text-gray-600 space-y-1">
                                {process.inputs.filter(i => i.isKey).slice(0, 5).map(input => (
                            ...
                            <div>
                              <h4 className="font-medium text-gray-900 mb-2">Key Tools & Techniques</h4>
                              <ul className="text-sm text-gray-600 space-y-1">
                                {process.toolsAndTechniques.filter(t => t.isKey).slice(0, 5).map(tool => (
                            ...
                            <div>
                              ...
                              <ul className="text-sm text-gray-600 space-y-1">
                                {process.outputs.filter(o => o.isKey).slice(0, 5).map(output => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredProcesses.map(process => {
                ...
                return (
                  <Card key={process.id} className={`hover:shadow-lg transition-all duration-200 ${
                    progress?.masteryLevel ? getMasteryColor(progress.masteryLevel) : ''
                  }`}>
                    <CardHeader className="pb-3">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        <Badge variant="outline" className="text-xs">{process.knowledgeArea}</Badge>
                        <Badge variant="secondary" className="text-xs">{process.processGroup}</Badge>
                        <Badge variant="outline" className="text-xs">PMBOK {process.version}</Badge>
                      </div>
                    </CardHeader>
                ...
                <Tabs defaultValue="overview" className="mt-4">
                  ...
                  <TabsContent value="overview" className="space-y-4">
                    ...
                    {selectedProcess.pmbok7Domains && selectedProcess.pmbok7Domains.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-lg mb-2">PMBOK 7 Performance Domains</h3>
                  ...
                  <TabsContent value="itto" className="space-y-6">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedProcess.inputs.map(input => (
                            <div key={input.id} className={`p-3 rounded border ${
                              input.isKey ? 'bg-blue-50 border-blue-200' : 'bg-gray-50 border-gray-200'
                            }`}>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-medium text-sm">{input.name}</span>
                                {input.isKey && <Star className="w-3 h-3 text-blue-600" />}
                              </div>
                      ...
                      <div>
                        ...
                        <div className="space-y-2">
                          {selectedProcess.toolsAndTechniques.map(tool => (
                            <div key={tool.id} className={`p-3 rounded border ${
                              tool.isKey ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200'
                            }`}>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-medium text-sm">{tool.name}</span>
                                {tool.isKey && <Star className="w-3 h-3 text-green-600" />}
                      ...
                      <div>
                        <h3 className="font-semibold text-lg mb-3 flex items-center gap-2">
                          <ArrowRight className="w-5 h-5 text-purple-600 transform rotate-180" />
                          Outputs ({selectedProcess.outputs.length})
                        </h3>
                        <div className="space-y-2">
                          {selectedProcess.outputs.map(output => (
                            <div key={output.id} className={`p-3 rounded border ${
                              output.isKey ? 'bg-purple-50 border-purple-200' : 'bg-gray-50 border-gray-200'
                            }`}>
                              <div className="flex items-center gap-2 mb-1">
                                <span className="font-medium text-sm">{output.name}</span>
                                {output.isKey && <Star className="w-3 h-3 text-purple-600" />}
                              </div>
                  ...
                  <TabsContent value="relationships" className="space-y-4">
                    {selectedProcess.relatedProcesses.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-lg mb-3">Related Processes</h3>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          {selectedProcess.relatedProcesses.map(relatedId => {
                            ...
                            return (
                              <div key={relatedId} className="p-3 border border-gray-200 rounded hover:bg-gray-50 cursor-pointer"
                                   onClick={() => setSelectedProcess(relatedProcess)}>
                                <div className="font-medium text-sm">{relatedProcess.name}</div>
                                <div className="text-xs text-gray-600">{relatedProcess.knowledgeArea}</div>
                              </div>
                            );
                          })}
                        </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-3">
                        <div className="p-4 bg-blue-50 border border-blue-200 rounded">
                          <h4 className="font-medium text-blue-900 mb-2">📚 Study Focus Areas</h4>
                        ...
                        <div className="p-4 bg-green-50 border border-green-200 rounded">
                          <h4 className="font-medium text-green-900 mb-2">⏱️ Estimated Study Time</h4>
                        ...
                        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded">
                          <h4 className="font-medium text-yellow-900 mb-2">🎯 Learning Tips</h4>
