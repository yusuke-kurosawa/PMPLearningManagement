...
const PMBOK7Principles = () => {
  ...
  return (
    <div className={`p-6 ${settings.darkMode ? 'bg-gray-900 text-white' : 'bg-gray-50'}`}>
      {/* ヘッダー */}
      <div className="mb-8">
        <h2 className="text-3xl font-bold mb-3">PMBOK第7版 - 12のプロジェクトマネジメント原則</h2>
            ...
            >
              <div className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div className={`p-3 rounded-lg ${getColorClasses(color, 'bg')}`}>
                ...
                {isExpanded && (
                  <div className="mt-4 pt-4 border-t dark:border-gray-700">
                    ...
                    <div className="space-y-1">
                      <h4 className="text-sm font-medium mb-2">主要なアクション:</h4>
                      {principle.keyActions.map((action, index) => (
                        ...
                        >
                          <span className={`mt-1 w-1.5 h-1.5 rounded-full ${getColorClasses(color, 'bg')}`} />
                          <span>{action}</span>
                        </div>
                      ))}
                    </div>
      ...
      {selectedPrinciple && (
        <div className={`fixed inset-0 z-50 overflow-y-auto`}>
          <div className="min-h-screen px-4 text-center">
            <div className="fixed inset-0 bg-black bg-opacity-50" onClick={() => setSelectedPrinciple(null)} />
              ...
              <div className={`rounded-lg shadow-xl ${
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-start gap-4">
                      <div className={`p-3 rounded-lg ${
                        getColorClasses(principleColors[selectedPrinciple.id], 'bg')
                      }`}>
                      ...
                      <div>
                        <h3 className="text-2xl font-bold mb-2">{selectedPrinciple.name}</h3>
                        <p className={`${settings.darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
                  ...
                  <div className="space-y-6">
                    ...
                    {/* 主要なアクション */}
                    <div>
                      <h4 className="font-semibold mb-3">実践のための主要なアクション</h4>
                          ...
                          >
                            <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${
                              ...
                              <span className={`text-xs font-semibold ${
                                getColorClasses(principleColors[selectedPrinciple.id], 'text')
                              }`}>
                                {index + 1}
                              </span>
                            </div>
