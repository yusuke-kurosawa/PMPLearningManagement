...
export const examDomains = {
  ...
  PROCESS: { id: 'process', name: 'Process', weight: 0.5 },
...
export const questionBank = [
  // People Domain (42%)
  {
    ...
    question:
      'プロジェクトチームが分散環境で作業している場合、チームの結束力を高めるために最も効果的な方法は次のうちどれですか？',
    options: [
    ...
    correctAnswer: 'A',
    explanation:
      '分散チームでは、定期的なビデオ会議によってチームメンバー間の関係性を構築し、コミュニケーションを促進することが重要です。これにより、チームの結束力が高まり、プロジェクトの成功確率が向上します。',
  ...
  },
  {
    ...
    type: questionTypes.SINGLE_CHOICE,
    question:
      'ステークホルダーが頻繁に要求を変更し、プロジェクトの範囲が拡大している。プロジェクトマネージャーとして最初に取るべき行動は？',
    options: [
    ...
    correctAnswer: 'B',
    explanation:
      '頻繁な変更要求の根本原因は、多くの場合、ステークホルダーの期待値が不明確であることです。まず期待値を明確にし、合意を得ることが重要です。',
  ...
  // Process Domain (50%)
  {
    ...
    correctAnswer: 'C',
    explanation:
      'プロジェクト憲章は高レベルの文書であり、詳細なWBSは含みません。WBSはプロジェクト計画フェーズで作成されます。',
  ...
  },
  {
    ...
    correctAnswer: 'B',
    explanation:
      'クリティカルパス法は、プロジェクトの最短完了時間を特定し、どのアクティビティが遅延するとプロジェクト全体が遅延するかを明確にします。',
  ...
  },
  {
    ...
    explanation:
      'モンテカルロ・シミュレーション、感度分析、デシジョンツリー分析は定量的リスク分析の技法です。SWOT分析は定性的な分析手法です。',
  ...
  // Business Environment Domain (8%)
  {
    ...
    correctAnswer: 'B',
    explanation:
      'ビジネスケースは、プロジェクトがどのように組織の戦略的目標を支援するかを文書化し、投資の正当性を示します。',
  ...
  },
  {
    ...
    correctAnswer: 'C',
    explanation:
      'プロジェクトマネジメント計画書のテンプレートは組織のプロセス資産（OPA）に分類されます。EEFは組織が直接コントロールできない要因です。',
...
export const generateExam = (questionCount = 180) => {
  ...
  const businessQuestions = shuffled
    .filter((q) => q.domain === 'business_environment')
    .slice(0, businessCount)
...
export const analyzeExamResults = (answers, questions) => {
  ...
  Object.keys(results.domainScores).forEach((domain) => {
    ...
    domainScore.percentage =
      domainScore.total > 0 ? Math.round((domainScore.correct / domainScore.total) * 100) : 0
  })

  Object.keys(results.knowledgeAreaScores).forEach((area) => {
    const areaScore = results.knowledgeAreaScores[area]
    areaScore.percentage =
      areaScore.total > 0 ? Math.round((areaScore.correct / areaScore.total) * 100) : 0
  })
