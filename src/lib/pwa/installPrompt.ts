...
class PWAInstallPromptManager {
  ...
  public canShowPrompt(): boolean {
    ...
    const daysSinceFirstVisit = (Date.now() - this.state.firstVisit.getTime()) / (1000 * 60 * 60 * 24);
    ...
    if (this.state.lastPromptShown) {
      const hoursSinceLastPrompt = (Date.now() - this.state.lastPromptShown.getTime()) / (1000 * 60 * 60);
      if (hoursSinceLastPrompt < 24) { // 24 hour cooldown
        return false;
      }
    }
...
export const getInstallPromptManager = (config?: Partial<InstallPromptConfig>): PWAInstallPromptManager => {
