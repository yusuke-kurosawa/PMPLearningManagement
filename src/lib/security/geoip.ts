...
export class GeoIPService {
  ...
  private calculateThreatScore(securityData: unknown): number {
    ...
    if (securityData?.is_proxy || securityData?.proxy) {
      score += 30
    }
    if (securityData?.is_vpn || securityData?.vpn) {
      score += 20
    }
    if (securityData?.is_tor || securityData?.tor) {
      score += 50
    }
    if (securityData?.is_hosting || securityData?.hosting) {
      score += 15
    }
    if (securityData?.is_anonymous_proxy) {
      score += 40
    }
    if (securityData?.threat_level === 'high') {
      score += 60
    }
    if (securityData?.threat_level === 'medium') {
      score += 30
    }
