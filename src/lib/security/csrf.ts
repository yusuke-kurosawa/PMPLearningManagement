...
export class CSRFProtection {
  ...
  async validateToken(
    ...
    try {
      ...
      // 異常に古い/新しいタイムスタンプの検知
      if (age < -5000 || age > this.config.tokenExpiry) { // 5秒の許容範囲
      ...
      if (providedSignature.length !== expectedSignature.length || 
          !timingSafeEqual(Buffer.from(providedSignature, 'hex'), Buffer.from(expectedSignature, 'hex'))) {
  ...
  private async storeToken(tokenValue: string, token: CSRFToken): Promise<void> {
    if (this.redis) {
      ...
      await this.redis.setex(
        key,
        Math.ceil(this.config.tokenExpiry / 1000),
  ...
  private async validateEnhancedRequest(
    ...
    let tokenFromHeader = request.headers[this.config.headerName] || 
                         request.headers[this.config.headerName.toLowerCase()];
      ...
      for (const userKey of userKeys) {
        ...
        for (const oldToken of tokens) {
          ...
          if (storedToken && storedToken.expiresAt > Date.now()) {
            ...
            console.info(`Token rotated for user ${userId}: ${oldToken.substring(0, 8)}... -> ${newToken.substring(0, 8)}...`);
          }
        }
      }
    } catch (error) {
...
export function useCSRFToken(userId?: string): {
  ...
  const protectedFetch = async (input: RequestInfo | URL, init: RequestInit = {}): Promise<Response> => {
    ...
    if (['POST', 'PUT', 'PATCH', 'DELETE'].includes(method)) {
      ...
      let tokens = csrfProtection.getTokenFromCookie();
      let token: string;
