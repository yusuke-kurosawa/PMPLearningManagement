...
export class DataSanitizer {
   ...
   */
  private static sanitizeHTML(html: string, options: {
    allowedTags: string[];
    allowedAttributes: string[];
  }): string {
    ...
    sanitized = sanitized.replace(/<(\/?)([\w-]+)([^>]*)>/g, (match, closing, tagName, attributes) => {
   ...
   */
  static sanitizeObject(obj: any, options: SanitizationOptions = {}): {
    sanitized: any;
    warnings: string[];
    modified: boolean;
  } {
    ...
    try {
      ...
      return {
        sanitized: {},
        warnings: [`Object sanitization failed: ${error instanceof Error ? error.message : 'Unknown error'}`],
        modified: true
      };
    }
  }
   ...
   */
  static batchSanitize(inputs: string[], options: SanitizationOptions = {}): {
    sanitized: string[];
