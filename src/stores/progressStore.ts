...
interface ProgressStore {
  ...
  createGoal: (goal: Omit<LearningGoal, 'id' | 'currentValue' | 'isCompleted' | 'createdAt'>) => Promise<void>;
    ...
    immer((set, get) => ({
      ...
      updateProcessProgress: async (processId: string, updates: Partial<ProcessProgress>) => {
        ...
        try {
          ...
          console.error('Failed to sync process progress:', error);
        }
      },
      ...
      createGoal: async (goalData) => {
        ...
        try {
          ...
          console.error('Failed to sync goal creation:', error);
        }
      },
      ...
      getMonthlyTrends: () => {
        ...
        const masteryProgression = last12Months.map(month => {
          ...
          const monthProcesses = processes.filter(p => 
            p.lastStudied && p.lastStudied.toISOString().slice(0, 7) === month
          );
          
          const masteryLevels = { not_started: 0, beginner: 1, intermediate: 2, advanced: 3, mastered: 4 };
          const averageMastery = monthProcesses.length > 0
            ? monthProcesses.reduce((sum, p) => sum + masteryLevels[p.masteryLevel], 0) / monthProcesses.length
            : 0;
      ...
      getOverallProgress: () => {
        ...
        const masteryLevels = { not_started: 0, beginner: 25, intermediate: 50, advanced: 75, mastered: 100 };
