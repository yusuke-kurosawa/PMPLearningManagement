...
interface FlashCardStore {
  ...
  createCard: (card: Omit<FlashCard, 'id' | 'createdAt' | 'updatedAt' | 'totalReviews' | 'correctReviews' | 'incorrectReviews' | 'averageResponseTime'>) => Promise<string>;
  ...
  createDeck: (deck: Omit<FlashCardDeck, 'id' | 'createdAt' | 'updatedAt' | 'totalCards' | 'masteredCards' | 'averageEaseFactor'>) => Promise<string>;
  ...
  startStudySession: (deckId: string, filters?: Partial<FlashCardStore['studyFilters']>) => Promise<void>;
  ...
  // Actions - Analytics
  getStudyStatistics: (deckId?: string, days?: number) => {
    ...
    immer((set, get) => ({
      ...
      loadDecks: async () => {
        ...
        try {
          ...
          set((state) => {
            state.decks = decks.reduce((acc, deck) => {
              acc[deck.id] = {
                ...deck,
      ...
      startStudySession: async (deckId: string, filters) => {
        ...
        if (effectiveFilters.knowledgeArea) {
          studyCards = studyCards.filter(card => card.knowledgeArea === effectiveFilters.knowledgeArea);
        }

        if (effectiveFilters.processGroup) {
          studyCards = studyCards.filter(card => card.processGroup === effectiveFilters.processGroup);
        }
      ...
      endStudySession: async () => {
        ...
        const totalTime = Math.floor((endTime.getTime() - currentSession.startTime.getTime()) / 1000);
          ...
          if (state.studySessions[0]) {
            ...
            state.studySessions[0].averageResponseTime = 
              state.studySessions[0].cardResults.length > 0
                ? state.studySessions[0].cardResults.reduce((sum, r) => sum + r.responseTime, 0) / state.studySessions[0].cardResults.length
                : 0;
          }
      ...
      updateSpacedRepetition: (cardId: string, quality: number, responseTime: number) => {
        ...
        set((state) => {
          ...
          const totalResponseTime = cardToUpdate.averageResponseTime * (cardToUpdate.totalReviews - 1) + responseTime;
        ...
        api.flashcards.updateSpacedRepetition.mutate({
          ...
          console.error('Failed to sync spaced repetition data:', error);
        });
      },
      ...
      getNewCards: (deckId?: string, limit = 20) => {
        ...
        return cards
          .filter(card => card.repetitions === 0 && card.isActive)
          .slice(0, limit);
      },
      ...
      getSpacedRepetitionStats: () => {
        ...
        const dueToday = cards.filter(card => 
          card.nextReviewDate <= today && card.isActive
        ).length;
        ...
        const mastered = cards.filter(card => 
          card.repetitions >= 5 && card.easeFactor >= 2.5 && card.isActive
        ).length;

        const learning = cards.filter(card => 
          card.repetitions > 0 && card.repetitions < 5 && card.isActive
        ).length;
      ...
      getStudyStatistics: (deckId?: string, days = 30) => {
        ...
        let sessions = get().studySessions.filter(s => 
          s.endTime && s.startTime >= cutoffDate
        );
