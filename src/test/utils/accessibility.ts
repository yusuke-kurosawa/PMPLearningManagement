...
export const testColorContrast = async (container: HTMLElement) => {
  ...
  const elementsWithBackgroundColor = container.querySelectorAll('[style*="background-color"], [style*="color"]');
...
export const testAriaPatterns = {
  ...
  tabpanel: (element: HTMLElement) => {
    ...
    if (tabs.length !== panels.length) {
      throw new Error(`Tab pattern: Number of tabs (${tabs.length}) doesn't match panels (${panels.length})`);
    }
