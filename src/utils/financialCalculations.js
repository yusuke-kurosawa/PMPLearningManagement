...
export const monteCarloSimulation = (params, iterations = 1000) => {
  ...
  for (let i = 0; i < iterations; i++) {
    ...
    const randomInitialInvestment =
      params.initialInvestment * (1 + (Math.random() - 0.5) * params.investmentVolatility)
    const randomCashFlows = params.cashFlows.map(
      (cf) => cf * (1 + (Math.random() - 0.5) * params.cashFlowVolatility)
    )
    const randomDiscountRate =
      params.discountRate * (1 + (Math.random() - 0.5) * params.rateVolatility)
