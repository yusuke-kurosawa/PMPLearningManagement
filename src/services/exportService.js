...
class ExportService {
  ...
  exportLearningReport() {
    ...
    const completedCount = Object.values(progress.processes || {})
  ...
  getKnowledgeAreaStats(progress) {
    ...
    Object.keys(areas).forEach(area => {
      areas[area].percentage = Math.round(
        (areas[area].completed / areas[area].total) * 100
      );
    });
  ...
  getProcessGroupStats(progress) {
    ...
    Object.keys(groups).forEach(group => {
      groups[group].percentage = Math.round(
        (groups[group].completed / groups[group].total) * 100
      );
    });
