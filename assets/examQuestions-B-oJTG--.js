...
const generateExam = /* @__PURE__ */ __name((questionCount = 180) => {
...
const analyzeExamResults = /* @__PURE__ */ __name((answers, questions) => {
  const results = {
  ...
  questions.forEach((question) => {
    ...
    if (!userAnswer) {
      results.unanswered++;
    } else if (question.type === questionTypes.SINGLE_CHOICE && userAnswer === question.correctAnswer || question.type === questionTypes.MULTIPLE_CHOICE && JSON.stringify(userAnswer.sort()) === JSON.stringify(question.correctAnswers.sort())) {
    ...
    if (question.knowledgeArea) {
      ...
      if (userAnswer && (question.type === questionTypes.SINGLE_CHOICE && userAnswer === question.correctAnswer || question.type === questionTypes.MULTIPLE_CHOICE && JSON.stringify(userAnswer.sort()) === JSON.stringify(question.correctAnswers.sort()))) {
  ...
  Object.keys(results.domainScores).forEach((domain) => {
    ...
    domainScore.percentage = domainScore.total > 0 ? Math.round(domainScore.correct / domainScore.total * 100) : 0;
  });
  Object.keys(results.knowledgeAreaScores).forEach((area) => {
    const areaScore = results.knowledgeAreaScores[area];
    areaScore.percentage = areaScore.total > 0 ? Math.round(areaScore.correct / areaScore.total * 100) : 0;
  });
  return results;
}, "analyzeExamResults");
export {
  analyzeExamResults as a,
  examDomains as e,
  generateExam as g,
  questionTypes as q
};
