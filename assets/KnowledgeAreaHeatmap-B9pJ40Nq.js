...
import { G as select, T as sequential, Y as YlOrRd, l as linear, U as axisBottom, V as format } from "./d3-core-DnNGvVRC.js";
import { g as glossaryService } from "./glossaryService-DZ07_k-f.js";
import { G as GlossaryDialog } from "./GlossaryDialog-C_TOA3GO.js";
import { T as TrendingUp, I as Activity, bl as TrendingDown, aL as Info } from "./lucide-icons-B7slfWYt.js";
...
const KnowledgeAreaHeatmap = React.memo(() => {
  ...
  const [selectedMetric, setSelectedMetric] = reactExports.useState("processCount");
  ...
  reactExports.useEffect(() => {
    ...
    const colorScale = sequential().interpolator(YlOrRd).domain([0, 5]);
    ...
    svg.append("text").attr("x", width / 2).attr("y", 40).attr("text-anchor", "middle").style("font-size", "24px").style("font-weight", "bold").style("fill", "#111827").text("知識エリア別ヒートマップ");
    svg.append("text").attr("x", width / 2).attr("y", 65).attr("text-anchor", "middle").style("font-size", "14px").style("fill", "#6b7280").text(((_a = metrics.find((m) => m.id === selectedMetric)) == null ? void 0 : _a.description) || "");
    g.selectAll(".process-group-label").data(processGroups).enter().append("text").attr("class", "process-group-label").attr("x", (d, i) => i * cellWidth + cellWidth / 2).attr("y", -10).attr("text-anchor", "middle").style("font-size", "14px").style("font-weight", "600").style("fill", "#374151").text((d) => d);
    g.selectAll(".knowledge-area-label").data(knowledgeAreas).enter().append("text").attr("class", "knowledge-area-label").attr("x", -10).attr("y", (d, i) => i * cellHeight + cellHeight / 2).attr("text-anchor", "end").attr("dominant-baseline", "middle").style("font-size", "13px").style("fill", "#374151").text((d) => d.name).style("cursor", "pointer").on("click", (event, d) => {
      const term = glossaryService.getTermByName(d.name);
      if (term) {
        setSelectedGlossaryTerm(term);
      }
    });
    const cells = g.selectAll(".heatmap-cell").data(
      knowledgeAreas.flatMap(
        (area, i) => processGroups.map((group, j) => ({
          area: area.id,
          areaName: area.name,
          group,
    ...
    ).enter().append("g").attr("class", "heatmap-cell");
    cells.append("rect").attr("x", (d) => d.x).attr("y", (d) => d.y).attr("width", cellWidth - 2).attr("height", cellHeight - 2).attr("rx", 4).attr("fill", (d) => d.value === 0 ? "#e5e7eb" : colorScale(d.value)).attr("stroke", "#fff").attr("stroke-width", 2).style("cursor", (d) => d.value > 0 ? "pointer" : "default").on("mouseenter", function(event, d) {
      if (d.value > 0) {
        select(this).transition().duration(200).attr("stroke", "#3b82f6").attr("stroke-width", 3);
        setHoveredCell(d);
      }
    }).on("mouseleave", function(_event, _d) {
      select(this).transition().duration(200).attr("stroke", "#fff").attr("stroke-width", 2);
      setHoveredCell(null);
    });
    cells.append("text").attr("x", (d) => d.x + cellWidth / 2).attr("y", (d) => d.y + cellHeight / 2).attr("text-anchor", "middle").attr("dominant-baseline", "middle").style("font-size", "20px").style("font-weight", "bold").style("fill", (d) => d.value > 3 ? "#fff" : "#374151").style("pointer-events", "none").text((d) => d.value > 0 ? d.value : "");
    const legendWidth = 200;
    const legendHeight = 20;
    const legendX = width - margin.right - legendWidth - 20;
    const legendY = margin.top - 80;
    const legendScale = linear().domain([0, 5]).range([0, legendWidth]);
    const legendAxis = axisBottom(legendScale).ticks(6).tickFormat(format("d"));
    const gradient = svg.append("defs").append("linearGradient").attr("id", "legend-gradient").attr("x1", "0%").attr("x2", "100%").attr("y1", "0%").attr("y2", "0%");
    for (let i = 0; i <= 10; i++) {
      gradient.append("stop").attr("offset", `${i * 10}%`).attr("stop-color", colorScale(i / 2));
    }
    ...
    legend.append("text").attr("x", legendWidth / 2).attr("y", -5).attr("text-anchor", "middle").style("font-size", "12px").style("fill", "#6b7280").text("値の範囲");
    const stats = calculateStats();
    const statsG = svg.append("g").attr("transform", `translate(${margin.left},${height - 40})`);
    statsG.selectAll(".stat").data([
    ...
    ]).enter().append("g").attr("class", "stat").attr("transform", (d, i) => `translate(${i * 150},0)`);
    function calculateStats() {
      const values = Object.values(heatmapData).flatMap(
        (area) => Object.values(area).filter((v) => v > 0)
      );
      return {
        max: Math.max(...values),
        min: Math.min(...values),
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex h-screen w-full flex-col bg-gray-50", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "border-b bg-white p-4 shadow-sm", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mx-auto max-w-7xl", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-6", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-2", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("label", { className: "text-sm font-medium text-gray-700", children: "表示指標:" }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex gap-2", children: metrics.map((metric) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
            onClick: /* @__PURE__ */ __name(() => setSelectedMetric(metric.id), "onClick"),
            className: `rounded-lg px-3 py-1.5 text-sm font-medium transition-colors ${selectedMetric === metric.id ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`,
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { ref: containerRef, className: "relative flex-1", children: [
      ...
      hoveredCell && /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "div",
        {
          ...
          children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-sm font-semibold", children: hoveredCell.areaName }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "text-xs opacity-90", children: hoveredCell.group }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-1 text-lg font-bold", children: hoveredCell.value })
          ]
        }
      )
    ] }),
...
export {
  KnowledgeAreaHeatmap as default
};
