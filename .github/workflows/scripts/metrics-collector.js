...
class MetricsCollector {
  ...
  async calculateWorkflowMetrics(workflowId) {
    ...
    const durations = completedRuns.map((r) => r.duration).filter((d) => d !== null)
  ...
  calculateOverallSummary(allMetrics) {
    ...
    const totalCost = allMetrics.reduce(
      (sum, m) => sum + parseFloat(m.cost.projectedMonthlyCost),
      0
    )
...
${metrics.summary.topPerformers
  .map((w, i) => `${i + 1}. ${w.name}: ${(w.successRate * 100).toFixed(1)}%`)
  .join('\n')}

## ⚠️ 改善が必要なワークフロー

${metrics.summary.bottomPerformers
  .map((w, i) => `${i + 1}. ${w.name}: ${(w.successRate * 100).toFixed(1)}%`)
  .join('\n')}
`)
