...
*このIssueは Claude Code Instruction Logger により自動生成されました*`
  ...
  determineLabels(instruction, metadata) {
    ...
    if (
      instructionLower.includes('bug') ||
      instructionLower.includes('fix') ||
      instructionLower.includes('エラー')
    ) {
      labels.push('bug')
    }

    if (
      instructionLower.includes('feature') ||
      instructionLower.includes('implement') ||
      instructionLower.includes('機能')
    ) {
    ...
    if (
      ...
      instructionLower.includes('testing') ||
      instructionLower.includes('テスト')
    ) {
      labels.push('testing')
    }

    if (
      instructionLower.includes('doc') ||
      instructionLower.includes('documentation') ||
      instructionLower.includes('ドキュメント')
    ) {
  ...
  getDefaultConfig() {
    return {
      ...
      excludePatterns: ['test', 'debug', 'temporary'],
      priorityKeywords: {
...
async function main() {
  if (require.main === module) {
    ...
    try {
      ...
      const issueNumber = await logger.logInstruction(
        instruction,
        response || '実行中...',
        metadata
      )
