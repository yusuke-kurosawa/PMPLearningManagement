...
class DocumentSyncValidator {
  ...
  async validateProjectInfo(claudeContent, readmeContent) {
    ...
    if (claudeTitle && readmeTitle) {
      // PMBOK バージョン情報の確認
      const claudeHasPMBOK6 =
        claudeContent.includes('PMBOK第6版') || claudeContent.includes('PMBOK第6版・第7版')
      const readmeHasPMBOK6 =
        readmeContent.includes('PMBOK第6版') || readmeContent.includes('PMBOK第6版・第7版')
  ...
  async validateLanguageConventions(claudeContent, readmeContent) {
    ...
    const claudeHasLanguageConvention =
      claudeContent.includes('言語規約') || claudeContent.includes('日本語.*標準')
    const readmeHasLanguageConvention =
      readmeContent.includes('言語規約') || readmeContent.includes('日本語.*標準')
    ...
    const claudeHasGitHubJapanese =
      claudeContent.includes('GitHub.*日本語') || claudeContent.includes('Issue.*日本語')
    const readmeHasGitHubJapanese =
      readmeContent.includes('GitHub.*日本語') || readmeContent.includes('Issue.*日本語')
  ...
  async validateDevelopmentInfo(claudeContent, readmeContent) {
    ...
    const devCommands = ['npm install', 'npm run dev', 'npm run build', 'npm run preview']
    ...
    if (claudePort && readmePort && claudePort !== readmePort) {
      this.errors.push(
        `開発サーバーポートが不一致です (CLAUDE: ${claudePort}, README: ${readmePort})`
      )
    }
  }
  ...
  outputResults() {
    ...
    console.log(
      `📏 CLAUDE.md: ${this.metrics.claudeLines}行 (${this.formatBytes(this.metrics.claudeSize)})`
    )
    console.log(
      `📏 README.md: ${this.metrics.readmeLines}行 (${this.formatBytes(this.metrics.readmeSize)})`
    )
