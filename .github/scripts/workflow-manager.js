...
class WorkflowManager {
  ...
  async getWorkflowRunHistory() {
    try {
      const output = execSync('gh run list --limit 50 --json conclusion,workflowName,createdAt,status', 
        { encoding: 'utf8' });
  ...
  findPatterns(workflows) {
    ...
    workflows.forEach(workflow => {
      // トリガーパターン
      const triggerMatches = workflow.content.match(/on:\s*\n([\s\S]*?)(?=\n[a-zA-Z]|\nenv:|\njobs:)/);
      if (triggerMatches) {
...
${analysis.duplicates.length === 0 ? 'No significant duplicates found.' : 
    ...
    if (analysis.duplicates.length > 0) {
      suggestions.push({
        ...
        details: analysis.duplicates.map(dup => 
          `- Merge ${dup.count} workflows running ${dup.type}: ${dup.files.join(', ')}`
        )
      });
    }
...
${suggestions.length === 0 ? '## ✅ No Major Issues Found\n\nYour workflows are well-optimized!' :
