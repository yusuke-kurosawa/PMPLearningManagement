...
const branchPattern =
  /^(feature|bugfix|hotfix|release|docs|refactor|test|ci|deps|experimental|chore|perf|security)\/(issue-\d+-)?.{1,50}$/
...
if (require.main === module) {
  const branchName =
    process.argv[2] ||
    require('child_process').execSync('git branch --show-current').toString().trim()
