  ...
  plugins: [
    {
      rules: {
        'issue-reference': ({subject, body, footer}) => {
          ...
          return [
            hasIssue,
            `Commit must reference an issue (e.g., #123)`
          ];
        }
      }
    }
...
class CommitValidator {
  ...
  validate(message) {
    ...
    if (firstLine.length > this.rules.maxSubjectLength) {
      errors.push(`Subject line too long: ${firstLine.length} chars (max: ${this.rules.maxSubjectLength})`);
    }
    ...
    if (pastTenseWords.includes(firstWord)) {
      errors.push(`Use imperative mood: "${firstWord.replace(/ed$/, '')}" instead of "${firstWord}"`);
    }
...
function generateCommitReport() {
  ...
  commits.forEach(commit => {
    ...
    const hasType = /^[a-f0-9]+ (feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert|security)/.test(commit);
