  ...
  analyze() {
    ...
    for (let i = 0; i < this.events.length - 1; i++) {
      this.events[i].duration = 
        this.events[i + 1].timestamp - this.events[i].timestamp;
    }
    ...
    const impactDuration = 
      this.events[this.events.length - 1].timestamp - trigger.timestamp;
...
class BlamelessPostmortem {
  ...
  reviewPostmortem(document) {
    ...
    const hasLearnings = document.includes('Lessons Learned') || 
                        document.includes('What We Learned');
    if (!hasLearnings) {
      issues.push('Missing lessons learned section');
    }
    
    // Check for action items
    const hasActions = document.includes('Action Items') || 
                       document.includes('Follow-up Actions');
    if (!hasActions) {
