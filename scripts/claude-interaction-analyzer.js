...
class ClaudeInteractionAnalyzer {
  ...
  generateInsights(analysis) {
    ...
    insights.push({
      ...
      recommendation:
        'Consider creating specialized workflows for frequently used interaction types.',
    })
...
`
  ...
  async analyze() {
    ...
    console.log(
      `- Most common type: ${Object.entries(analysis.byType).sort(([, a], [, b]) => b - a)[0]?.[0] || 'N/A'}`
    )
    console.log(
      `- High priority rate: ${(((analysis.byPriority.critical || 0) + (analysis.byPriority.high || 0)) / analysis.total) * 100}%`
    )
