...
class MetricsCollector {
  ...
  async collectGitHubActionsMetrics() {
    ...
    try {
      // 直近のワークフロー実行を取得
      const runsJson = execSync('gh run list --limit 100 --json status,conclusion,createdAt,updatedAt,name', {
        encoding: 'utf-8'
      });
  ...
  async collectRepositoryMetrics() {
    ...
    try {
      ...
      const commitCount = execSync(`git rev-list --count --since="${thirtyDaysAgo.toISOString()}" HEAD`, {
        encoding: 'utf-8'
      }).trim();
