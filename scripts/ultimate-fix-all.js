...
function getESLintStats() {
  try {
    const output = exec('npx eslint src --ext .js,.jsx,.ts,.tsx --format json', { silent: true, ignoreError: true })
...
async function main() {
  ...
  log.info(`検出された問題: ${colors.red}${beforeStats.errors} エラー${colors.reset}, ${colors.yellow}${beforeStats.warnings} 警告${colors.reset}`)
  ...
  log.info('インポート文を整理中...')
  exec('npx eslint src --ext .js,.jsx,.ts,.tsx --fix --rule "import/order: error"', { silent: true, ignoreError: true })
