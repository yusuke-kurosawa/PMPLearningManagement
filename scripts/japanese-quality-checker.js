...
const JAPANESE_QUALITY_STANDARDS = {
  ...
  // 品質しきい値
  QUALITY_THRESHOLDS: {
    grammar_accuracy: 0.90,      // 90%以上の文法正確性
    terminology_consistency: 0.95, // 95%以上の用語一貫性
    readability: 0.85,           // 85%以上の読みやすさ
    cultural_appropriateness: 0.90 // 90%以上の文化的適切性
  }
};
...
class JapaneseQualityChecker {
  ...
  async checkJapaneseQuality() {
    ...
    try {
      ...
      console.log(`✅ 日本語品質チェック完了 - スコア: ${this.qualityResults.overall_score.toFixed(2)}%`);
    ...
    for (const file of files) {
      ...
      if (stats.isFile() && (file.endsWith('.jsx') || file.endsWith('.tsx'))) {
        try {
          ...
          // JSX内のテキストを抽出
          const textMatches = content.match(/>[^<>]*[\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FAF][^<>]*</g) || [];
    ...
    allTexts.forEach((textItem, index) => {
      ...
      JAPANESE_GRAMMAR_PATTERNS.PROBLEMATIC_PATTERNS.forEach(rule => {
        ...
        matches.forEach(match => {
          ...
          // スコア調整
          const penalty = rule.severity === 'high' ? 0.05 : 
                         rule.severity === 'medium' ? 0.02 : 0.01;
      ...
      // 文が長すぎる場合の減点
      if (sentenceAnalysis.avg_sentence_length > JAPANESE_QUALITY_STANDARDS.GRAMMAR_RULES.sentence_length.max) {
        grammarScore -= 0.02;
      }
    });
  ...
  analyzeSentenceStructure(text) {
    ...
    return {
      ...
      long_sentences: sentences.filter(s => s.length > JAPANESE_QUALITY_STANDARDS.GRAMMAR_RULES.sentence_length.max).length,
  ...
  async checkTerminologyConsistency(contentData) {
    ...
    // PMI公式用語の使用状況チェック
    Object.entries(JAPANESE_QUALITY_STANDARDS.PMI_OFFICIAL_TERMS.technical_terms).forEach(([japanese, english]) => {
      let usageCount = 0;
    ...
    if (terminologyAnalysis.inconsistent_usage.length > 0) {
      this.addTerminologyIssue('MEDIUM', `用語の揺れが${terminologyAnalysis.inconsistent_usage.length}箇所で検出されました`);
    }
  ...
  async evaluateReadability(contentData) {
    ...
    if (sentenceVariety.variety_score < READABILITY_METRICS.JAPANESE_READABILITY.sentence_variety.min) {
    ...
    // 説明的要素のスコア
    if (educationalElements.explanation_ratio >= READABILITY_METRICS.EDUCATIONAL_READABILITY.explanation_ratio.min) {
      readabilityScore += 0.05;
    }
    
    if (educationalElements.example_ratio >= READABILITY_METRICS.EDUCATIONAL_READABILITY.example_ratio.min) {
      readabilityScore += 0.05;
    }
  ...
  evaluateBusinessAppropriateness(allTexts) {
    const businessTerms = [
      'プロジェクト', 'マネジメント', '組織', '戦略', 'プロセス',
      'ステークホルダー', 'リソース', 'コスト', '品質', 'リスク'
    ];
  ...
  evaluateEducationalTone(allTexts) {
    ...
    const totalIndicators = Object.values(educationalIndicators).reduce((sum, count) => sum + count, 0);
  ...
  evaluateCulturalSensitivity(allTexts) {
    ...
    return {
      ...
      cultural_sensitivity_score: Object.values(sensitivityIndicators).reduce((sum, count) => sum + count, 0) / Math.max(allTexts.length, 1),
  ...
  calculateOverallScore() {
    const weights = {
      grammar_accuracy: 0.30,           // 30% - 文法正確性
      terminology_consistency: 0.25,   // 25% - 用語一貫性
      readability: 0.25,               // 25% - 読みやすさ
      cultural_appropriateness: 0.20   // 20% - 文化的適切性
    };
  ...
  generateJapaneseQualityReport() {
    ...
    return {
      ...
      quality_gates: {
        grammar_gate: this.qualityResults.grammar_accuracy >= JAPANESE_QUALITY_STANDARDS.QUALITY_THRESHOLDS.grammar_accuracy,
        terminology_gate: this.qualityResults.terminology_consistency >= JAPANESE_QUALITY_STANDARDS.QUALITY_THRESHOLDS.terminology_consistency,
        readability_gate: this.qualityResults.readability >= JAPANESE_QUALITY_STANDARDS.QUALITY_THRESHOLDS.readability,
        cultural_gate: this.qualityResults.cultural_appropriateness >= JAPANESE_QUALITY_STANDARDS.QUALITY_THRESHOLDS.cultural_appropriateness
      },
