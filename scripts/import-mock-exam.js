...
const questionSchema = {
  ...
  optional: [
    'question_domain', 'question_category', 'difficulty_level',
    'explanation', 'references', 'tags', 'scenario_context',
    'time_estimate_seconds', 'is_calculation', 'pmbok_version'
  ],
  domains: ['people', 'process', 'business_environment'],
  categories: [
    'agile_adaptive', 'predictive_waterfall', 'hybrid', 'leadership',
    'team_performance', 'communication', 'risk_management', 'quality_management',
    'resource_management', 'stakeholder_engagement', 'integration_management',
    'scope_management', 'schedule_management', 'cost_management',
    'procurement_management', 'ethics_professional_conduct'
  ],
  ...
  for (const block of blocks) {
    ...
    for (const line of lines) {
      ...
      // Difficulty
        ...
        const diff = trimmedLine.replace(/^Difficulty:/i, '').trim().toLowerCase();
        question.difficulty_level = mapDifficulty(diff);
      }
      // References
      else if (trimmedLine.match(/^References?:/i)) {
        question.references = trimmedLine.replace(/^References?:/i, '').trim().split(';');
      }
  ...
  questions.forEach((q, index) => {
    ...
    if (q.options) {
      ...
      if (q.question_type === 'single' && correctOptions.length !== 1) {
        errors.push(`Question ${questionNum}: Single choice question must have exactly 1 correct answer`);
      }
      if (q.question_type === 'multiple' && correctOptions.length < 1) {
        errors.push(`Question ${questionNum}: Multiple choice question must have at least 1 correct answer`);
      }
    }
  });
  ...
  try {
    ...
    for (const question of questions) {
      try {
        ...
        imported++;
        console.log(`✅ Imported question ${imported}: ${question.question_text.substring(0, 50)}...`);
