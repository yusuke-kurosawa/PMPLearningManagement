...
// プレフィックス付きラベル設計を読み込み
const prefixedLabelDesign = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'prefixed-fun-label-system-design.json'), 'utf8')
)
...
function migrateToPrefixedLabels() {
  ...
  for (const [oldLabel, newLabel] of Object.entries(migrationRules)) {
    ...
    const checkResult = runCommand(
      `gh api repos/${REPO}/labels/${encodeURIComponent(oldLabel)} 2>/dev/null`
    )
    if (checkResult.success) {
      ...
      const searchResult = runCommand(
        `gh issue list --repo ${REPO} --label "${oldLabel}" --limit 100 --json number`
      )
      if (searchResult.success) {
        ...
        for (const issue of issues) {
          const removeResult = runCommand(
            `gh issue edit ${issue.number} --remove-label "${oldLabel}" --repo ${REPO}`
          )
          const addResult = runCommand(
            `gh issue edit ${issue.number} --add-label "${newLabel}" --repo ${REPO}`
          )
  ...
  logWithEmoji(
    '📊',
    `移行結果: 作成/更新 ${createdCount}個, 移行 ${migratedCount}個, エラー ${errorCount}個`
  )
...
function showPrefixedLabelStatistics() {
  ...
  const totalLabels = Object.values(prefixedLabelDesign.labelCategories).reduce(
...
function updateIssueTemplates() {
  ...
  templateMappings.forEach((mapping) => {
    ...
    if (fs.existsSync(templatePath)) {
      ...
      const updatedContent = content.replace(/^labels:\s*\[.*?\]$/m, `labels: ${mapping.labels}`)
