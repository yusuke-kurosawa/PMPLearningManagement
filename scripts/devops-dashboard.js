...
class DevOpsDashboard {
  ...
  async collectCodeQualityMetrics() {
    ...
    try {
      // ESLint metrics
      const { stdout: eslintOutput } = await execAsync('npx eslint src --format json', { encoding: 'utf8' }).catch(() => ({ stdout: '[]' }));
      ...
      // Code complexity
      const { stdout: complexityOutput } = await execAsync('find src -name "*.js" -o -name "*.jsx" -o -name "*.ts" -o -name "*.tsx" | wc -l');
      const totalSourceFiles = parseInt(complexityOutput.trim());
      
      // Lines of code
      const { stdout: locOutput } = await execAsync('find src -name "*.js" -o -name "*.jsx" -o -name "*.ts" -o -name "*.tsx" -exec wc -l {} + | tail -1');
...
- **Info**: ${vulns.info || 0}`;
  ...
  generateRecommendations() {
    ...
    if (this.metrics.codeQuality.eslint?.errors > 0) {
      recommendations.push(`🔴 **Critical**: Fix ${this.metrics.codeQuality.eslint.errors} ESLint errors`);
    }
    
    if (this.metrics.codeQuality.eslint?.warnings > 100) {
      recommendations.push(`🟡 **Warning**: Address ${this.metrics.codeQuality.eslint.warnings} ESLint warnings`);
    }
    
    // Test coverage recommendations
    if (this.metrics.testMetrics.lineCoverage && this.metrics.testMetrics.lineCoverage < 80) {
      recommendations.push(`🟡 **Testing**: Improve test coverage (currently ${this.metrics.testMetrics.lineCoverage}%)`);
    }
    
    // Security recommendations
    if (this.metrics.securityMetrics.vulnerabilities?.critical > 0) {
      recommendations.push(`🔴 **Security**: Fix ${this.metrics.securityMetrics.vulnerabilities.critical} critical vulnerabilities immediately`);
    }
    
    if (this.metrics.securityMetrics.vulnerabilities?.high > 0) {
      recommendations.push(`🟠 **Security**: Address ${this.metrics.securityMetrics.vulnerabilities.high} high severity vulnerabilities`);
    }
