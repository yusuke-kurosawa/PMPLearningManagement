...
class AgentCoordinationSystem {
  ...
  async executeQualityCheck(action, context) {
    const actions = {
      ...
      'security-scan': () => ({
        outputs: {
          vulnerabilities: [
            { id: 'CVE-2023-1234', severity: 'low', package: 'lodash@4.17.19' }
          ],
          riskScore: 2.1
        },
  ...
  getSystemHealth() {
    ...
    const avgSuccessRate = activeAgents.reduce((sum, agent) => 
      sum + agent.performance.successRate, 0) / activeAgents.length;

    const avgResponseTime = activeAgents.reduce((sum, agent) => 
      sum + agent.performance.avgResponseTime, 0) / activeAgents.length;
...
if (process.argv[1] === new URL(import.meta.url).pathname) {
  ...
  setInterval(() => {
    ...
    console.log(`🏥 System Health: ${health.systemStatus} (${health.activeAgents}/${health.totalAgents} agents active)`);
  }, 30000);
