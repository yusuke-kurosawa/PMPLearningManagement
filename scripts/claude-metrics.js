...
class ClaudeMetricsCollector {
  ...
  parsePeriod(period) {
    ...
    console.log(`📅 Analysis period: ${this.startDate.toISOString().split('T')[0]} to ${this.endDate.toISOString().split('T')[0]}`);
  }
  ...
  async collectCommentData() {
    ...
    for (const item of allItems) {
      try {
        ...
        this.rawData.comments.push(...relevantComments.map(comment => ({
          ...comment,
  ...
  async calculateIssueMetrics() {
    ...
    // Issue creation rate
    const createdInPeriod = issues.filter(issue => 
      new Date(issue.created_at) >= this.startDate
    ).length;
    this.metrics.issueCreationRate = (createdInPeriod / daysInPeriod).toFixed(2);

    // Issue resolution rate
    const closedInPeriod = issues.filter(issue => 
      issue.state === 'closed' && 
    ...
    if (closedIssues.length > 0) {
      const totalCycleTime = closedIssues.reduce((sum, issue) => {
        const cycleTime = (new Date(issue.closed_at) - new Date(issue.created_at)) / (1000 * 60 * 60 * 24);
  ...
  async calculatePRMetrics() {
    ...
    // PR creation rate
    const createdInPeriod = prs.filter(pr => 
      new Date(pr.created_at) >= this.startDate
    ).length;
    ...
    if (prsWithReviews.length > 0) {
      ...
      this.metrics.averageReviewTime = (totalReviewTime / prsWithReviews.length / (1000 * 60 * 60)).toFixed(1); // hours
    } else {
    ...
    if (mergedPRs.length > 0) {
      ...
      this.metrics.averageMergeTime = (totalMergeTime / mergedPRs.length / (1000 * 60 * 60 * 24)).toFixed(1); // days
    } else {
  ...
  async calculateReviewTimes(prs) {
    ...
    for (const pr of prs.slice(0, 20)) { // Limit to avoid rate limits
      try {
  ...
  async calculateQualityMetrics() {
    ...
    this.metrics.securityIssueRate = issues.length > 0 ? ((securityIssues / issues.length) * 100).toFixed(1) : 0;

    // Code quality indicators
    const enhancementIssues = issues.filter(issue => 
      issue.labels.some(label => label.name.toLowerCase() === 'enhancement')
    ).length;
    this.metrics.enhancementRate = issues.length > 0 ? ((enhancementIssues / issues.length) * 100).toFixed(1) : 0;

    // Test coverage (would need integration with coverage tools)
    // For now, we'll infer from test-related issues/PRs
    const testRelatedItems = [...issues, ...this.rawData.pullRequests].filter(item => 
  ...
  async calculateAIMetrics() {
    ...
    this.metrics.aiResponseRate = totalItems > 0 ? ((aiComments.length / totalItems) * 100).toFixed(1) : 0;

    // Time to AI response
    const aiResponseTimes = [];
    this.rawData.issues.forEach(issue => {
      const issueComments = comments.filter(comment => 
        comment.item_number === issue.number && comment.item_type === 'issue'
      );
      
      const aiComment = issueComments.find(comment => 
        comment.body && comment.body.includes('Claude AI Assistant')
      );
    ...
    if (aiResponseTimes.length > 0) {
      const avgResponseTime = aiResponseTimes.reduce((sum, time) => sum + time, 0) / aiResponseTimes.length;
    ...
    const userAIInteractions = comments.filter(comment => 
  ...
  async calculateVelocityMetrics() {
    ...
    this.metrics.velocityPercentage = totalStoryPoints > 0 ? 
    ...
    this.metrics.issuesPerDeveloper = activeDevelopers.size > 0 ? 
      (this.rawData.issues.length / activeDevelopers.size).toFixed(1) : 0;
