/**
 * Issue品質チェッカー
 * GitHub Actionsから実行されるIssue品質監査スクリプト
 */
...
class IssueQualityChecker {
  ...
  checkTitle(issue) {
    ...
    // 長さチェック
    if (title.length < this.qualityStandards.title.minLength) {
      result.valid = false
      result.issues.push(
        `タイトルが短すぎます（${title.length}文字 < ${this.qualityStandards.title.minLength}文字）`
      )
    } else {
      result.score += 20
    }

    if (title.length > this.qualityStandards.title.maxLength) {
      result.issues.push(
        `タイトルが長すぎます（${title.length}文字 > ${this.qualityStandards.title.maxLength}文字）`
      )
      result.score -= 10
    }

    // 禁止パターンチェック
    const forbiddenMatch = this.qualityStandards.title.forbiddenPatterns.find((pattern) =>
      pattern.test(title)
    )
    if (forbiddenMatch) {
      result.valid = false
    ...
    // タグパターンチェック
    const hasProperTag = Object.values(this.qualityStandards.title.requiredPatterns).some(
  ...
  checkDescription(issue) {
    ...
    // 長さチェック
    if (body.length < this.qualityStandards.description.minLength) {
      result.valid = false
      result.issues.push(
        `説明文が短すぎます（${body.length}文字 < ${this.qualityStandards.description.minLength}文字）`
      )
    } else {
      result.score += 30
    }
    ...
    const requiredSections = this.qualityStandards.description.requiredSections[issueType] || []
  ...
  checkLabels(issue) {
    ...
    const hasPriority = labels.some((label) =>
      this.qualityStandards.labels.priority.includes(label)
    )
    if (hasPriority) {
      result.score += 30
    } else {
    ...
    const hasComponent = labels.some((label) =>
      this.qualityStandards.labels.component.some((prefix) => label.startsWith(prefix))
    )
    if (hasComponent) {
      result.score += 25
    } else {
    ...
    // タイプラベルチェック
    const hasType = labels.some((label) => this.qualityStandards.labels.type.includes(label))
    if (hasType) {
      result.score += 25
    } else {
  ...
  checkSecurityCompliance(issue) {
    ...
    if (result.isSecurityIssue) {
      ...
      const hasSensitiveData = sensitivePattens.some((pattern) => pattern.test(body))
      if (hasSensitiveData) {
  ...
  calculateQualityScore(checks) {
    ...
    for (const [category, check] of Object.entries(checks)) {
      if (weights[category] && typeof check.score === 'number') {
   ...
   * 改善提案生成
   */
  generateRecommendations(checks, issue) {
    ...
    for (const [category, check] of Object.entries(checks)) {
      if (check.issues && check.issues.length > 0) {
        recommendations.push({
          category: category,
          priority: this.getRecommendationPriority(category, check),
          issues: check.issues,
   ...
   * 推奨事項の優先度計算
   */
  getRecommendationPriority(category, check) {
    const priorities = {
      security: 100,
      priority: 90,
      labels: 80,
   ...
   * 改善提案の生成
   */
  getImprovementSuggestions(category, check, issue) {
    const suggestions = {
      title: [
  ...
  checkSLACompliance(issue) {
    ...
    let slaHours = this.qualityStandards.sla['priority:low'] // デフォルト

    // 最も厳しいSLAを適用
    for (const label of labels) {
      if (this.qualityStandards.sla[label]) {
        slaHours = Math.min(slaHours, this.qualityStandards.sla[label])
      }
    }
   ...
   * 品質レポート生成
   */
  generateQualityReport(issue, qualityResult) {
...
module.exports = { IssueQualityChecker }
