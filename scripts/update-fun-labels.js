...
// 楽しいラベル設計を読み込み
const funLabelDesign = JSON.parse(fs.readFileSync(path.join(__dirname, 'fun-label-system-design.json'), 'utf8'));
...
function showLabelStatistics() {
  ...
  const totalLabels = Object.values(funLabelDesign.labelCategories)
  ...
  console.log(`\n📊 総ラベル数: ${totalLabels}個`);
  console.log(`🌟 ゲーミフィケーション要素: ${Object.keys(funLabelDesign.gamification.features).length}種類`);
  console.log(`🤝 コミュニティ原則: ${Object.keys(funLabelDesign.community_guidelines.principles).length}項目`);
}
