...
async function checkPostgresHealth() {
  ...
  try {
    ...
    const result = await prisma.$queryRaw`SELECT version(), current_database(), pg_postmaster_start_time()`;
...
async function generateHealthReport() {
  ...
  if (pgHealth.details) {
    ...
    if (pgHealth.details.connections) {
      console.log(`  Connections: ${pgHealth.details.connections.total_connections} total, ${pgHealth.details.connections.active_connections} active`);
    }
    if (pgHealth.details.tables) {
      console.log(`  Tables: ${pgHealth.details.tables.total_tables} tables, ${pgHealth.details.tables.total_rows} rows`);
    }
