...
class LighthouseTrendAnalyzer {
  ...
  identifyPerformanceIssues(latest) {
    ...
    if (resourceMetrics.totalByteWeight > 2048000) { // 2MB
      issues.push({
        type: 'large-bundle',
        severity: 'high',
    ...
    if (resourceMetrics.unusedJavaScript > 200000) { // 200KB
      issues.push({
  ...
  async saveAnalysis(data) {
    ...
    const summary = {
      ...
      bundleSize: Math.round((data.results[data.results.length - 1]?.resourceMetrics.totalByteWeight || 0) / 1024 / 1024 * 100) / 100,
      bundleSizeStatus: this.getBundleSizeStatus(data.results[data.results.length - 1]?.resourceMetrics.totalByteWeight || 0),
      loadTime: (data.results[data.results.length - 1]?.performanceMetrics.tti / 1000 || 0).toFixed(1),
      loadTimeStatus: this.getLoadTimeStatus(data.results[data.results.length - 1]?.performanceMetrics.tti || 0),
      reportUrl: 'https://lighthouse-results.example.com'
    };
    
    fs.writeFileSync('./lighthouse-results/performance-summary.json', JSON.stringify(summary, null, 2));
  }
...
## Current State
${Object.entries(analysis.currentState.status).map(([category, status]) => 
  `- **${category}**: ${status} (${analysis.currentState.scores[category]}/100)`
).join('\\n')}

## Core Web Vitals
${Object.entries(analysis.coreWebVitalsStatus).map(([metric, data]) =>
