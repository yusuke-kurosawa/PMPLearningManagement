  ...
  const fixes = [
    ...
    // connectionPool.ts:106 - ',' expected
    {
      ...
      fix: (content) => {
        ...
        if (lines[105] && !lines[105].trim().endsWith(',') && !lines[105].trim().endsWith(';') && !lines[105].trim().endsWith('{') && !lines[105].trim().endsWith('}')) {
    ...
    // csrf.ts:521 - ',' expected
    {
      ...
      fix: (content) => {
        ...
        if (lines[520]) {
          ...
          // Add comma if needed
          if (!lines[520].trim().endsWith(',') && !lines[520].trim().endsWith('}') && lines[521] && !lines[521].trim().startsWith('}')) {
            lines[520] = lines[520] + ',';
          }
        }
        return lines.join('\n');
    ...
    // encryptedUserService.ts:27 - ';' expected
    {
      ...
      fix: (content) => {
        ...
        if (lines[26] && !lines[26].trim().endsWith(';') && !lines[26].trim().endsWith('{') && !lines[26].trim().endsWith('}')) {
          lines[26] = lines[26] + ';';
        }
        return lines.join('\n');
      }
