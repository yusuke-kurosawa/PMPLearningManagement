...
const PMBOK_VALIDATION_RULES = {
  ...
  QUALITY_THRESHOLDS: {
    ...
    itto_consistency: 0.90,      // 90%以上のITTO整合性
...
class PMBOKValidator {
  ...
  async validateProject() {
    ...
    try {
      ...
      console.log(`✅ PMBOK準拠性検証完了 - スコア: ${this.validationResults.overall_score.toFixed(2)}%`);
  ...
  validateProcessCompleteness(processData) {
    ...
    Object.entries(PMBOK_VALIDATION_RULES.PROCESS_GROUPS).forEach(([pg, range]) => {
      ...
      if (actual < range.min || actual > range.max) {
        this.addIssue('MEDIUM', `${pg}プロセス群: ${actual}プロセス (推奨範囲: ${range.min}-${range.max})`);
      ...
      if (process.itto?.inputs) {
        process.itto.inputs.forEach(input => {
          ...
          if (!isExternalInput && !inputOutputMap.has(input)) {
            unreferencedInputs++;
            this.addIssue('LOW', 
              `プロセス ${process.name}: インプット「${input}」の出所が不明`);
          }
        });
      }
    });
  ...
  generateComplianceReport() {
    ...
    return {
      ...
      quality_gates: {
        process_gate: this.validationResults.process_completeness >= PMBOK_VALIDATION_RULES.QUALITY_THRESHOLDS.process_completeness,
        itto_gate: this.validationResults.itto_consistency >= PMBOK_VALIDATION_RULES.QUALITY_THRESHOLDS.itto_consistency,
        terminology_gate: this.validationResults.terminology_accuracy >= PMBOK_VALIDATION_RULES.QUALITY_THRESHOLDS.terminology_accuracy,
        relationship_gate: this.validationResults.relationship_validity >= PMBOK_VALIDATION_RULES.QUALITY_THRESHOLDS.relationship_validity
      },
  ...
  generateNextActions() {
    ...
    if (this.validationResults.itto_consistency < 0.90) {
