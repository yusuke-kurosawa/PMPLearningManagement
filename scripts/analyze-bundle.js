...
class IntelligentBundleAnalyzer {
  ...
  analyzeChunks(fileSizes) {
    ...
    return {
      ...
      oversizedChunks: jsFiles
        .filter((f) => f.size > this.budgets.chunkSize)
        .map((f) => ({
          path: f.path,
  ...
  findDuplications(dependencyAnalysis) {
    ...
    return {
      ...
      potentialSaving: duplicates.reduce(
        (sum, dup) =>
          sum + Math.min(...dup.versions.map((v) => nameVersionMap.get(dup.name)?.size || 0)),
        0
      ),
    }
  }
  ...
  async generateReports(analysis) {
    ...
    const summary = {
      ...
      budgetStatus:
        analysis.summary.totalSizeBytes > this.budgets.totalSize ? 'exceeded' : 'compliant',
