...
class DuplicateDetector {
    ...
    for (let i = 0; i < issues.length; i++) {
      ...
      for (let j = i + 1; j < issues.length; j++) {
        ...
        if (similarity.overall >= this.detectionConfig.thresholds.overall) {
          ...
          // より新しいIssueを重複としてマーク
          const newerIssue = new Date(currentIssue.created_at) > new Date(compareIssue.created_at) 
            ? currentIssue : compareIssue;
          processed.add(newerIssue.number);
        }

        // 進捗表示
        if ((i * issues.length + j) % 100 === 0) {
          console.log(`📊 進捗: ${Math.round(((i * issues.length + j) / (issues.length * issues.length)) * 100)}%`);
        }
      }
    }
  ...
  identifyPotentialDuplicates(duplicatePairs) {
    return duplicatePairs.filter(pair => 
      pair.confidence >= 0.5 && pair.confidence < 0.75
    );
  }
  ...
  async handleDuplicatePair(duplicatePair) {
    ...
    try {
      ...
      console.error(`❌ 重複ペア処理エラー (Issues #${issue1.number}, #${issue2.number}):`, error.message);
    }
  }
  ...
  async autoCloseDuplicate(duplicateIssue, originalIssue, similarity, confidence) {
    ...
    // 重複コメント追加
    const comment = this.generateDuplicateComment(duplicateIssue, originalIssue, similarity, confidence, 'auto-closed');
  ...
  async flagForManualReview(duplicateIssue, originalIssue, similarity, confidence) {
    ...
    // レビューコメント追加
    const comment = this.generateDuplicateComment(duplicateIssue, originalIssue, similarity, confidence, 'manual-review');
    await this.github.rest.issues.createComment({
      owner: this.context.repo.owner,
      repo: this.context.repo.repo,
      issue_number: duplicateIssue.number,
    ...
    console.log(`🔍 手動レビュー要求: Issue #${duplicateIssue.number} (類似: #${originalIssue.number})`);
  }
  ...
  async flagAsPotentialDuplicate(duplicateIssue, originalIssue, similarity) {
    ...
    // 潜在的重複コメント追加
    const comment = this.generateDuplicateComment(duplicateIssue, originalIssue, similarity, null, 'potential');
    await this.github.rest.issues.createComment({
      owner: this.context.repo.owner,
      repo: this.context.repo.repo,
      issue_number: duplicateIssue.number,
    ...
    console.log(`⚠️  潜在的重複フラグ: Issue #${duplicateIssue.number} (類似: #${originalIssue.number})`);
  }
  ...
  async generateDuplicateReport(results) {
    ...
    if (results.duplicatePairs.length > 0) {
      ...
      for (const pair of results.duplicatePairs.slice(0, 20)) { // 上位20件
