...
class DependencyOptimizer {
  ...
  async runOptimization() {
    ...
    try {
      ...
      console.log(`💾 潜在的ディスク節約: ${this.formatBytes(this.results.potentialSavings.diskSpace)}`);
      console.log(`📦 潜在的バンドル削減: ${this.formatBytes(this.results.potentialSavings.bundleSize)}`);
  ...
  async loadPackageData() {
    ...
    try {
      ...
      console.log(`✓ 依存関係: ${this.results.summary.totalDependencies}本体, ${this.results.summary.totalDevDependencies}開発`);
    ...
    for (const [scriptName, script] of Object.entries(scripts)) {
      ...
      for (const command of commands) {
        ...
        if (words.length > 0) {
          ...
          if (this.packageJson.devDependencies?.[tool] || 
              this.packageJson.dependencies?.[tool]) {
            references.add(tool);
          }
        }
      }
  ...
  async analyzeBundleImpact() {
    ...
    try {
      ...
      this.results.bundleAnalysis = {
        ...
        averagePackageSize: packageSizes.length > 0 ? 
          packageSizes.reduce((sum, pkg) => sum + pkg.size, 0) / packageSizes.length : 0
      };

      console.log(`✓ 総サイズ: ${this.formatBytes(totalSize)}, 最大パッケージ: ${packageSizes[0]?.name || 'N/A'}`);
  ...
  generateRecommendations() {
    ...
    if (this.results.unusedDependencies.length > 0) {
      const totalUnusedSize = this.results.unusedDependencies
        .reduce((sum, dep) => sum + dep.estimatedSize, 0);
    ...
    // 古い依存関係の更新推奨
    const highRiskOutdated = this.results.outdatedDependencies
    ...
    // ライセンス問題への対処
    const copyLeftLicenses = this.results.licenseIssues
      .filter(issue => issue.issue === 'copyleft');
  ...
  calculatePotentialSavings() {
    // ディスク容量の節約
    this.results.potentialSavings.diskSpace = this.results.unusedDependencies
      .reduce((sum, dep) => sum + dep.estimatedSize, 0);
    ...
    // ビルド時間の節約（推定）
    const unusedDevDeps = this.results.unusedDependencies
      .filter(dep => dep.type === 'devDependency').length;
...
<body>
    <div class="container">
        ...
        <div class="content">
            ...
            <div class="section">
                <h2>🗑️ 未使用依存関係</h2>
                <div class="item-list">
                    ${this.results.unusedDependencies.map(dep => `
                        <div class="item">
                            <h4>${dep.name}</h4>
            ...
            <div class="section">
                <h2>📅 古い依存関係</h2>
                <div class="item-list">
                    ${this.results.outdatedDependencies.map(dep => `
  ...
  generateSummaryReport() {
    const unusedCount = this.results.unusedDependencies.length;
    const outdatedCount = this.results.outdatedDependencies.length;
    const highRiskCount = this.results.outdatedDependencies
      .filter(dep => dep.securityRisk === 'high').length;
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  optimizer.runOptimization()
    .then(results => {
      console.log('\n🎉 依存関係最適化完了!');
      console.log(`💾 潜在的ディスク節約: ${optimizer.formatBytes(results.potentialSavings.diskSpace)}`);
      console.log(`📦 潜在的バンドル削減: ${optimizer.formatBytes(results.potentialSavings.bundleSize)}`);
      console.log(`🏗️ 潜在的ビルド時間削減: ${results.potentialSavings.buildTime}秒`);
      
      // 未使用依存関係が多い場合は警告
      if (results.unusedDependencies.length > 5) {
        console.log(`⚠️ ${results.unusedDependencies.length} の未使用依存関係があります - クリーンアップを推奨`);
        process.exit(1);
      }
