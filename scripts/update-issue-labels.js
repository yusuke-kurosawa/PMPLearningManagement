...
// ラベル設計を読み込み
const labelDesign = JSON.parse(
  fs.readFileSync(path.join(__dirname, 'label-system-design.json'), 'utf8')
)
...
function getExistingIssues() {
  ...
  const result = runCommand(
    `gh issue list --repo ${REPO} --state all --limit 100 --json number,title,labels,state`
  )
  if (!result.success) {
